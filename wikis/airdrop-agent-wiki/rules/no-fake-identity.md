...
## Purpose
This page gives agents a stable rule pattern for no fake identity in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Fake Identity should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no fake identity to convert a vague request into explicit fields, checks, and boundaries.
- Use no fake identity as a checklist item before the agent produces a final answer.
- Use no fake identity during review to separate supported observations from unresolved risks.
- Use no fake identity to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no fake identity as permission to invent missing details.
