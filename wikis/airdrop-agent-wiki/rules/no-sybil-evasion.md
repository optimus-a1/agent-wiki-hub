...
## Purpose
This page gives agents a stable rule pattern for no sybil evasion in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Sybil Evasion should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no sybil evasion to convert a vague request into explicit fields, checks, and boundaries.
- Use no sybil evasion as a checklist item before the agent produces a final answer.
- Use no sybil evasion during review to separate supported observations from unresolved risks.
- Use no sybil evasion to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no sybil evasion as permission to invent missing details.
