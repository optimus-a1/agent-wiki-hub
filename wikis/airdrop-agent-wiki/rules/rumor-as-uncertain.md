...
## Purpose
This page gives agents a stable rule pattern for rumor as uncertain in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Rumor As Uncertain should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use rumor as uncertain to convert a vague request into explicit fields, checks, and boundaries.
- Use rumor as uncertain as a checklist item before the agent produces a final answer.
- Use rumor as uncertain during review to separate supported observations from unresolved risks.
- Use rumor as uncertain to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating rumor as uncertain as permission to invent missing details.
