...
## Purpose
This page gives agents a stable concept pattern for approval hygiene in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Approval Hygiene should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use approval hygiene to convert a vague request into explicit fields, checks, and boundaries.
- Use approval hygiene as a checklist item before the agent produces a final answer.
- Use approval hygiene during review to separate supported observations from unresolved risks.
- Use approval hygiene to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating approval hygiene as permission to invent missing details.
