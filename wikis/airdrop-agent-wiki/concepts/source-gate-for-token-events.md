---
title: "Source Gate For Token Events"
...
# Source Gate For Token Events

## Purpose
This page gives agents a stable concept pattern for source gate for token events in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Source Gate For Token Events should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use source gate for token events to convert a vague request into explicit fields, checks, and boundaries.
- Use source gate for token events as a checklist item before the agent produces a final answer.
- Use source gate for token events during review to separate supported observations from unresolved risks.
- Use source gate for token events to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating source gate for token events as permission to invent missing details.
