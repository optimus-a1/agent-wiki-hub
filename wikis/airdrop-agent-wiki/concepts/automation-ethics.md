...
## Purpose
This page gives agents a stable concept pattern for automation ethics in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Automation Ethics should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use automation ethics to convert a vague request into explicit fields, checks, and boundaries.
- Use automation ethics as a checklist item before the agent produces a final answer.
- Use automation ethics during review to separate supported observations from unresolved risks.
- Use automation ethics to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating automation ethics as permission to invent missing details.
