...
## Purpose
This page gives agents a stable concept pattern for phishing signal review in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Phishing Signal Review should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use phishing signal review to convert a vague request into explicit fields, checks, and boundaries.
- Use phishing signal review as a checklist item before the agent produces a final answer.
- Use phishing signal review during review to separate supported observations from unresolved risks.
- Use phishing signal review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating phishing signal review as permission to invent missing details.
