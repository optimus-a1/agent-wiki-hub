...
## Purpose
This page gives agents a stable concept pattern for task classification in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Task Classification should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use task classification to convert a vague request into explicit fields, checks, and boundaries.
- Use task classification as a checklist item before the agent produces a final answer.
- Use task classification during review to separate supported observations from unresolved risks.
- Use task classification to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating task classification as permission to invent missing details.
