...
# Signing Risk Classes

## Purpose
This page gives agents a stable concept pattern for signing risk classes in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Signing Risk Classes should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use signing risk classes to convert a vague request into explicit fields, checks, and boundaries.
- Use signing risk classes as a checklist item before the agent produces a final answer.
- Use signing risk classes during review to separate supported observations from unresolved risks.
- Use signing risk classes to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating signing risk classes as permission to invent missing details.
