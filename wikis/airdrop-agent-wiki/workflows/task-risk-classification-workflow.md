---
title: "Task Risk Classification Workflow"
...
# Task Risk Classification Workflow

## Purpose
This page gives agents a stable workflow pattern for task risk classification workflow in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Task Risk Classification Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Human Gate
Require human confirmation before wallet connection, signing, approval, identity-sensitive action, fund movement, or task that may violate platform rules.

## Source Gate
Project status, TGE, listing, snapshot, eligibility, funding, task rules, and contract addresses require source review before use.

## Examples
- Use task risk classification workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use task risk classification workflow as a checklist item before the agent produces a final answer.
- Use task risk classification workflow during review to separate supported observations from unresolved risks.
- Use task risk classification workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating task risk classification workflow as permission to invent missing details.
