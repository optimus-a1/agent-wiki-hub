...
## Purpose
This page gives agents a stable workflow pattern for approval hygiene review in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Approval Hygiene Review should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use approval hygiene review to convert a vague request into explicit fields, checks, and boundaries.
- Use approval hygiene review as a checklist item before the agent produces a final answer.
- Use approval hygiene review during review to separate supported observations from unresolved risks.
- Use approval hygiene review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating approval hygiene review as permission to invent missing details.
