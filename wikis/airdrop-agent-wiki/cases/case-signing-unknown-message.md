---
title: "Case Signing Unknown Message"
...
# Case Signing Unknown Message

## Purpose
This page gives agents a stable case pattern for case signing unknown message in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Signing Unknown Message should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case signing unknown message to convert a vague request into explicit fields, checks, and boundaries.
- Use case signing unknown message as a checklist item before the agent produces a final answer.
- Use case signing unknown message during review to separate supported observations from unresolved risks.
- Use case signing unknown message to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case signing unknown message as permission to invent missing details.
