---
title: "Case Phishing Domain Signal"
...
# Case Phishing Domain Signal

## Purpose
This page gives agents a stable case pattern for case phishing domain signal in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Phishing Domain Signal should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case phishing domain signal to convert a vague request into explicit fields, checks, and boundaries.
- Use case phishing domain signal as a checklist item before the agent produces a final answer.
- Use case phishing domain signal during review to separate supported observations from unresolved risks.
- Use case phishing domain signal to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case phishing domain signal as permission to invent missing details.
