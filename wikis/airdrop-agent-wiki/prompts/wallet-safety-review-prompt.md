---
title: "Wallet Safety Review Prompt"
...
# Wallet Safety Review Prompt

## Purpose
This page gives agents a stable prompt pattern for wallet safety review prompt in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Wallet Safety Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use wallet safety review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use wallet safety review prompt as a checklist item before the agent produces a final answer.
- Use wallet safety review prompt during review to separate supported observations from unresolved risks.
- Use wallet safety review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating wallet safety review prompt as permission to invent missing details.
