---
title: "Source Gate Airdrop Prompt"
...
# Source Gate Airdrop Prompt

## Purpose
This page gives agents a stable prompt pattern for source gate airdrop prompt in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Source Gate Airdrop Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use source gate airdrop prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use source gate airdrop prompt as a checklist item before the agent produces a final answer.
- Use source gate airdrop prompt during review to separate supported observations from unresolved risks.
- Use source gate airdrop prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating source gate airdrop prompt as permission to invent missing details.
