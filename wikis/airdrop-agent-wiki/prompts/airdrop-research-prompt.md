...
# Airdrop Research Prompt

## Purpose
This page gives agents a stable prompt pattern for airdrop research prompt in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Airdrop Research Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use airdrop research prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use airdrop research prompt as a checklist item before the agent produces a final answer.
- Use airdrop research prompt during review to separate supported observations from unresolved risks.
- Use airdrop research prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating airdrop research prompt as permission to invent missing details.
