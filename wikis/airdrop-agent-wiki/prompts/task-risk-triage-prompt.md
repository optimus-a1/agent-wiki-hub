...
# Task Risk Triage Prompt

## Purpose
This page gives agents a stable prompt pattern for task risk triage prompt in `airdrop-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Task Risk Triage Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use task risk triage prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use task risk triage prompt as a checklist item before the agent produces a final answer.
- Use task risk triage prompt during review to separate supported observations from unresolved risks.
- Use task risk triage prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating task risk triage prompt as permission to invent missing details.
