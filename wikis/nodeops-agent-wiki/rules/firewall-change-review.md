...
## Purpose
This page gives agents a stable rule pattern for firewall change review in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Firewall Change Review should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use firewall change review to convert a vague request into explicit fields, checks, and boundaries.
- Use firewall change review as a checklist item before the agent produces a final answer.
- Use firewall change review during review to separate supported observations from unresolved risks.
- Use firewall change review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating firewall change review as permission to invent missing details.
