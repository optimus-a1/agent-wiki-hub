...
## Purpose
This page gives agents a stable rule pattern for backup before mutation in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Backup Before Mutation should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use backup before mutation to convert a vague request into explicit fields, checks, and boundaries.
- Use backup before mutation as a checklist item before the agent produces a final answer.
- Use backup before mutation during review to separate supported observations from unresolved risks.
- Use backup before mutation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating backup before mutation as permission to invent missing details.
