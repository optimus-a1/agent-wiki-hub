---
title: "Destructive Command Gate"
...
## Purpose
This page gives agents a stable rule pattern for destructive command gate in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Destructive Command Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use destructive command gate to convert a vague request into explicit fields, checks, and boundaries.
- Use destructive command gate as a checklist item before the agent produces a final answer.
- Use destructive command gate during review to separate supported observations from unresolved risks.
- Use destructive command gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating destructive command gate as permission to invent missing details.
