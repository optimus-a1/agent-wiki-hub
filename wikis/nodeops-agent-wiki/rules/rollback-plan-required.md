...
## Purpose
This page gives agents a stable rule pattern for rollback plan required in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Rollback Plan Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use rollback plan required to convert a vague request into explicit fields, checks, and boundaries.
- Use rollback plan required as a checklist item before the agent produces a final answer.
- Use rollback plan required during review to separate supported observations from unresolved risks.
- Use rollback plan required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating rollback plan required as permission to invent missing details.
