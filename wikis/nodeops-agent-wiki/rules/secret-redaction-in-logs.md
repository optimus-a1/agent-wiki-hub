...
## Purpose
This page gives agents a stable rule pattern for secret redaction in logs in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Secret Redaction In Logs should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use secret redaction in logs to convert a vague request into explicit fields, checks, and boundaries.
- Use secret redaction in logs as a checklist item before the agent produces a final answer.
- Use secret redaction in logs during review to separate supported observations from unresolved risks.
- Use secret redaction in logs to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating secret redaction in logs as permission to invent missing details.
