---
title: "Provider Limit Source Gate"
...
# Provider Limit Source Gate

## Purpose
This page gives agents a stable rule pattern for provider limit source gate in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Provider Limit Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use provider limit source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use provider limit source gate as a checklist item before the agent produces a final answer.
- Use provider limit source gate during review to separate supported observations from unresolved risks.
- Use provider limit source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating provider limit source gate as permission to invent missing details.
