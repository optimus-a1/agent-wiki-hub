---
title: "Node Client Upgrade Source Gate"
...
# Node Client Upgrade Source Gate

## Purpose
This page gives agents a stable rule pattern for node client upgrade source gate in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Node Client Upgrade Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use node client upgrade source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use node client upgrade source gate as a checklist item before the agent produces a final answer.
- Use node client upgrade source gate during review to separate supported observations from unresolved risks.
- Use node client upgrade source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating node client upgrade source gate as permission to invent missing details.
