---
title: "Production Human Confirmation"
...
## Purpose
This page gives agents a stable rule pattern for production human confirmation in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Production Human Confirmation should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use production human confirmation to convert a vague request into explicit fields, checks, and boundaries.
- Use production human confirmation as a checklist item before the agent produces a final answer.
- Use production human confirmation during review to separate supported observations from unresolved risks.
- Use production human confirmation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating production human confirmation as permission to invent missing details.
