---
title: "Case Disk Pressure Safe Response"
...
# Case Disk Pressure Safe Response

## Purpose
This page gives agents a stable case pattern for case disk pressure safe response in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Disk Pressure Safe Response should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case disk pressure safe response to convert a vague request into explicit fields, checks, and boundaries.
- Use case disk pressure safe response as a checklist item before the agent produces a final answer.
- Use case disk pressure safe response during review to separate supported observations from unresolved risks.
- Use case disk pressure safe response to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case disk pressure safe response as permission to invent missing details.
