---
title: "Case Node Upgrade Source Gate"
...
# Case Node Upgrade Source Gate

## Purpose
This page gives agents a stable case pattern for case node upgrade source gate in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Node Upgrade Source Gate should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case node upgrade source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use case node upgrade source gate as a checklist item before the agent produces a final answer.
- Use case node upgrade source gate during review to separate supported observations from unresolved risks.
- Use case node upgrade source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case node upgrade source gate as permission to invent missing details.
