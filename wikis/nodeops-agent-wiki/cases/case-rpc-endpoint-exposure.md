---
title: "Case RPC Endpoint Exposure"
...
# Case RPC Endpoint Exposure

## Purpose
This page gives agents a stable case pattern for case rpc endpoint exposure in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case RPC Endpoint Exposure should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case rpc endpoint exposure to convert a vague request into explicit fields, checks, and boundaries.
- Use case rpc endpoint exposure as a checklist item before the agent produces a final answer.
- Use case rpc endpoint exposure during review to separate supported observations from unresolved risks.
- Use case rpc endpoint exposure to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case rpc endpoint exposure as permission to invent missing details.
