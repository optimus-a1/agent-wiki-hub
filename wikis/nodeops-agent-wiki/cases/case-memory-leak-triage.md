...
# Case Memory Leak Triage

## Purpose
This page gives agents a stable case pattern for case memory leak triage in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Memory Leak Triage should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case memory leak triage to convert a vague request into explicit fields, checks, and boundaries.
- Use case memory leak triage as a checklist item before the agent produces a final answer.
- Use case memory leak triage during review to separate supported observations from unresolved risks.
- Use case memory leak triage to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case memory leak triage as permission to invent missing details.
