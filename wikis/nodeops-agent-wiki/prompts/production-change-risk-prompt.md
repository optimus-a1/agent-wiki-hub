---
title: "Production Change Risk Prompt"
...
# Production Change Risk Prompt

## Purpose
This page gives agents a stable prompt pattern for production change risk prompt in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Production Change Risk Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use production change risk prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use production change risk prompt as a checklist item before the agent produces a final answer.
- Use production change risk prompt during review to separate supported observations from unresolved risks.
- Use production change risk prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating production change risk prompt as permission to invent missing details.
