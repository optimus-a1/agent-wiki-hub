---
title: "Ops Incident Triage Prompt"
...
# Ops Incident Triage Prompt

## Purpose
This page gives agents a stable prompt pattern for ops incident triage prompt in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Ops Incident Triage Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use ops incident triage prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use ops incident triage prompt as a checklist item before the agent produces a final answer.
- Use ops incident triage prompt during review to separate supported observations from unresolved risks.
- Use ops incident triage prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating ops incident triage prompt as permission to invent missing details.
