---
title: "Node Health Review Prompt"
...
# Node Health Review Prompt

## Purpose
This page gives agents a stable prompt pattern for node health review prompt in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Node Health Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use node health review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use node health review prompt as a checklist item before the agent produces a final answer.
- Use node health review prompt during review to separate supported observations from unresolved risks.
- Use node health review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating node health review prompt as permission to invent missing details.
