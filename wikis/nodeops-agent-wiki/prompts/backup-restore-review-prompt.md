---
title: "Backup Restore Review Prompt"
...
# Backup Restore Review Prompt

## Purpose
This page gives agents a stable prompt pattern for backup restore review prompt in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Backup Restore Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use backup restore review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use backup restore review prompt as a checklist item before the agent produces a final answer.
- Use backup restore review prompt during review to separate supported observations from unresolved risks.
- Use backup restore review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating backup restore review prompt as permission to invent missing details.
