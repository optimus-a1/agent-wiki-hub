...
## Purpose
This page gives agents a stable concept pattern for disk pressure signals in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Disk Pressure Signals should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use disk pressure signals to convert a vague request into explicit fields, checks, and boundaries.
- Use disk pressure signals as a checklist item before the agent produces a final answer.
- Use disk pressure signals during review to separate supported observations from unresolved risks.
- Use disk pressure signals to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating disk pressure signals as permission to invent missing details.
