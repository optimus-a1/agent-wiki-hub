...
## Purpose
This page gives agents a stable concept pattern for memory pressure signals in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Memory Pressure Signals should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use memory pressure signals to convert a vague request into explicit fields, checks, and boundaries.
- Use memory pressure signals as a checklist item before the agent produces a final answer.
- Use memory pressure signals during review to separate supported observations from unresolved risks.
- Use memory pressure signals to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating memory pressure signals as permission to invent missing details.
