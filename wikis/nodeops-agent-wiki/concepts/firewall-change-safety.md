...
## Purpose
This page gives agents a stable concept pattern for firewall change safety in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Firewall Change Safety should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use firewall change safety to convert a vague request into explicit fields, checks, and boundaries.
- Use firewall change safety as a checklist item before the agent produces a final answer.
- Use firewall change safety during review to separate supported observations from unresolved risks.
- Use firewall change safety to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating firewall change safety as permission to invent missing details.
