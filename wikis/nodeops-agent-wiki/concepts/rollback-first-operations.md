---
title: "Rollback First Operations"
...
## Purpose
This page gives agents a stable concept pattern for rollback first operations in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Rollback First Operations should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use rollback first operations to convert a vague request into explicit fields, checks, and boundaries.
- Use rollback first operations as a checklist item before the agent produces a final answer.
- Use rollback first operations during review to separate supported observations from unresolved risks.
- Use rollback first operations to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating rollback first operations as permission to invent missing details.
