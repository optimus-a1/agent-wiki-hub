...
## Purpose
This page gives agents a stable concept pattern for docker volume backup in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Docker Volume Backup should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use docker volume backup to convert a vague request into explicit fields, checks, and boundaries.
- Use docker volume backup as a checklist item before the agent produces a final answer.
- Use docker volume backup during review to separate supported observations from unresolved risks.
- Use docker volume backup to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating docker volume backup as permission to invent missing details.
