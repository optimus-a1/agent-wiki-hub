...
## Purpose
This page gives agents a stable concept pattern for network port diagnosis in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Network Port Diagnosis should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use network port diagnosis to convert a vague request into explicit fields, checks, and boundaries.
- Use network port diagnosis as a checklist item before the agent produces a final answer.
- Use network port diagnosis during review to separate supported observations from unresolved risks.
- Use network port diagnosis to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating network port diagnosis as permission to invent missing details.
