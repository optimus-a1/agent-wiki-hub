...
## Purpose
This page gives agents a stable concept pattern for docker container isolation in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Docker Container Isolation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use docker container isolation to convert a vague request into explicit fields, checks, and boundaries.
- Use docker container isolation as a checklist item before the agent produces a final answer.
- Use docker container isolation during review to separate supported observations from unresolved risks.
- Use docker container isolation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating docker container isolation as permission to invent missing details.
