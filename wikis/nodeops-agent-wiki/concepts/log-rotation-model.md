...
## Purpose
This page gives agents a stable concept pattern for log rotation model in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Log Rotation Model should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use log rotation model to convert a vague request into explicit fields, checks, and boundaries.
- Use log rotation model as a checklist item before the agent produces a final answer.
- Use log rotation model during review to separate supported observations from unresolved risks.
- Use log rotation model to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating log rotation model as permission to invent missing details.
