...
## Purpose
This page gives agents a stable concept pattern for linux service lifecycle in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Linux Service Lifecycle should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use linux service lifecycle to convert a vague request into explicit fields, checks, and boundaries.
- Use linux service lifecycle as a checklist item before the agent produces a final answer.
- Use linux service lifecycle during review to separate supported observations from unresolved risks.
- Use linux service lifecycle to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating linux service lifecycle as permission to invent missing details.
