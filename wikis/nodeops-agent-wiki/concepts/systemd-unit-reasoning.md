...
# Systemd Unit Reasoning

## Purpose
This page gives agents a stable concept pattern for systemd unit reasoning in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Systemd Unit Reasoning should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use systemd unit reasoning to convert a vague request into explicit fields, checks, and boundaries.
- Use systemd unit reasoning as a checklist item before the agent produces a final answer.
- Use systemd unit reasoning during review to separate supported observations from unresolved risks.
- Use systemd unit reasoning to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating systemd unit reasoning as permission to invent missing details.
