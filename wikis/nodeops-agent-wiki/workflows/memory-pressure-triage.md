...
## Purpose
This page gives agents a stable workflow pattern for memory pressure triage in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Memory Pressure Triage should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use memory pressure triage to convert a vague request into explicit fields, checks, and boundaries.
- Use memory pressure triage as a checklist item before the agent produces a final answer.
- Use memory pressure triage during review to separate supported observations from unresolved risks.
- Use memory pressure triage to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating memory pressure triage as permission to invent missing details.
