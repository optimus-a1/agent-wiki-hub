...
## Purpose
This page gives agents a stable workflow pattern for network port triage in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Network Port Triage should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use network port triage to convert a vague request into explicit fields, checks, and boundaries.
- Use network port triage as a checklist item before the agent produces a final answer.
- Use network port triage during review to separate supported observations from unresolved risks.
- Use network port triage to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating network port triage as permission to invent missing details.
