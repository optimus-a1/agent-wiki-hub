---
title: "Systemd Service Recovery"
...
## Purpose
This page gives agents a stable workflow pattern for systemd service recovery in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Systemd Service Recovery should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use systemd service recovery to convert a vague request into explicit fields, checks, and boundaries.
- Use systemd service recovery as a checklist item before the agent produces a final answer.
- Use systemd service recovery during review to separate supported observations from unresolved risks.
- Use systemd service recovery to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating systemd service recovery as permission to invent missing details.
