---
title: "Post Incident Review Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for post incident review workflow in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Post Incident Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use post incident review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use post incident review workflow as a checklist item before the agent produces a final answer.
- Use post incident review workflow during review to separate supported observations from unresolved risks.
- Use post incident review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating post incident review workflow as permission to invent missing details.
