---
title: "Node Client Upgrade Checklist"
...
# Node Client Upgrade Checklist

## Purpose
This page gives agents a stable workflow pattern for node client upgrade checklist in `nodeops-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Node Client Upgrade Checklist should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use node client upgrade checklist to convert a vague request into explicit fields, checks, and boundaries.
- Use node client upgrade checklist as a checklist item before the agent produces a final answer.
- Use node client upgrade checklist during review to separate supported observations from unresolved risks.
- Use node client upgrade checklist to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating node client upgrade checklist as permission to invent missing details.
