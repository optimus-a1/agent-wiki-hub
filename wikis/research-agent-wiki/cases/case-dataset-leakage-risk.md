---
title: "Case Dataset Leakage Risk"
...
# Case Dataset Leakage Risk

## Purpose
This page gives agents a stable case pattern for case dataset leakage risk in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Dataset Leakage Risk should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case dataset leakage risk to convert a vague request into explicit fields, checks, and boundaries.
- Use case dataset leakage risk as a checklist item before the agent produces a final answer.
- Use case dataset leakage risk during review to separate supported observations from unresolved risks.
- Use case dataset leakage risk to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case dataset leakage risk as permission to invent missing details.
