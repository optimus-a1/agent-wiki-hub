...
## Purpose
This page gives agents a stable case pattern for case unsupported claim in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Unsupported Claim should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case unsupported claim to convert a vague request into explicit fields, checks, and boundaries.
- Use case unsupported claim as a checklist item before the agent produces a final answer.
- Use case unsupported claim during review to separate supported observations from unresolved risks.
- Use case unsupported claim to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case unsupported claim as permission to invent missing details.
