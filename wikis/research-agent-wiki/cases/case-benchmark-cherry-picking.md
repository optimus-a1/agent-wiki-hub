---
title: "Case Benchmark Cherry Picking"
...
# Case Benchmark Cherry Picking

## Purpose
This page gives agents a stable case pattern for case benchmark cherry picking in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Benchmark Cherry Picking should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case benchmark cherry picking to convert a vague request into explicit fields, checks, and boundaries.
- Use case benchmark cherry picking as a checklist item before the agent produces a final answer.
- Use case benchmark cherry picking during review to separate supported observations from unresolved risks.
- Use case benchmark cherry picking to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case benchmark cherry picking as permission to invent missing details.
