---
title: "Case Strong Summary With Limitations"
...
# Case Strong Summary With Limitations

## Purpose
This page gives agents a stable case pattern for case strong summary with limitations in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Strong Summary With Limitations should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case strong summary with limitations to convert a vague request into explicit fields, checks, and boundaries.
- Use case strong summary with limitations as a checklist item before the agent produces a final answer.
- Use case strong summary with limitations during review to separate supported observations from unresolved risks.
- Use case strong summary with limitations to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case strong summary with limitations as permission to invent missing details.
