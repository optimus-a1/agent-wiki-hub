...
## Purpose
This page gives agents a stable workflow pattern for citation audit workflow in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Citation Audit Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use citation audit workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use citation audit workflow as a checklist item before the agent produces a final answer.
- Use citation audit workflow during review to separate supported observations from unresolved risks.
- Use citation audit workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating citation audit workflow as permission to invent missing details.
