---
title: "Dataset Limitations Review"
...
# Dataset Limitations Review

## Purpose
This page gives agents a stable workflow pattern for dataset limitations review in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dataset Limitations Review should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use dataset limitations review to convert a vague request into explicit fields, checks, and boundaries.
- Use dataset limitations review as a checklist item before the agent produces a final answer.
- Use dataset limitations review during review to separate supported observations from unresolved risks.
- Use dataset limitations review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dataset limitations review as permission to invent missing details.
