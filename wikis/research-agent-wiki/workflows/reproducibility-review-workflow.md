---
title: "Reproducibility Review Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for reproducibility review workflow in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Reproducibility Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use reproducibility review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use reproducibility review workflow as a checklist item before the agent produces a final answer.
- Use reproducibility review workflow during review to separate supported observations from unresolved risks.
- Use reproducibility review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating reproducibility review workflow as permission to invent missing details.
