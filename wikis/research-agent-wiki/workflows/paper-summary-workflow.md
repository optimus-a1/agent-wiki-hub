...
## Purpose
This page gives agents a stable workflow pattern for paper summary workflow in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Paper Summary Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use paper summary workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use paper summary workflow as a checklist item before the agent produces a final answer.
- Use paper summary workflow during review to separate supported observations from unresolved risks.
- Use paper summary workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating paper summary workflow as permission to invent missing details.
