...
## Purpose
This page gives agents a stable prompt pattern for paper summary prompt in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Paper Summary Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use paper summary prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use paper summary prompt as a checklist item before the agent produces a final answer.
- Use paper summary prompt during review to separate supported observations from unresolved risks.
- Use paper summary prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating paper summary prompt as permission to invent missing details.
