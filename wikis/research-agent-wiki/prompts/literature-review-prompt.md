...
## Purpose
This page gives agents a stable prompt pattern for literature review prompt in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Literature Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use literature review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use literature review prompt as a checklist item before the agent produces a final answer.
- Use literature review prompt during review to separate supported observations from unresolved risks.
- Use literature review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating literature review prompt as permission to invent missing details.
