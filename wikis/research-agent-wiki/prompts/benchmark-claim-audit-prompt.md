---
title: "Benchmark Claim Audit Prompt"
...
# Benchmark Claim Audit Prompt

## Purpose
This page gives agents a stable prompt pattern for benchmark claim audit prompt in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Benchmark Claim Audit Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use benchmark claim audit prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use benchmark claim audit prompt as a checklist item before the agent produces a final answer.
- Use benchmark claim audit prompt during review to separate supported observations from unresolved risks.
- Use benchmark claim audit prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating benchmark claim audit prompt as permission to invent missing details.
