---
title: "Citation Integrity Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for citation integrity prompt in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Citation Integrity Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use citation integrity prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use citation integrity prompt as a checklist item before the agent produces a final answer.
- Use citation integrity prompt during review to separate supported observations from unresolved risks.
- Use citation integrity prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating citation integrity prompt as permission to invent missing details.
