---
title: "Dataset License Source Gate"
...
# Dataset License Source Gate

## Purpose
This page gives agents a stable rule pattern for dataset license source gate in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dataset License Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use dataset license source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use dataset license source gate as a checklist item before the agent produces a final answer.
- Use dataset license source gate during review to separate supported observations from unresolved risks.
- Use dataset license source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dataset license source gate as permission to invent missing details.
