...
## Purpose
This page gives agents a stable rule pattern for uncertainty required in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Uncertainty Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use uncertainty required to convert a vague request into explicit fields, checks, and boundaries.
- Use uncertainty required as a checklist item before the agent produces a final answer.
- Use uncertainty required during review to separate supported observations from unresolved risks.
- Use uncertainty required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating uncertainty required as permission to invent missing details.
