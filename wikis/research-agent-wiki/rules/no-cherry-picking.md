...
## Purpose
This page gives agents a stable rule pattern for no cherry picking in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Cherry Picking should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no cherry picking to convert a vague request into explicit fields, checks, and boundaries.
- Use no cherry picking as a checklist item before the agent produces a final answer.
- Use no cherry picking during review to separate supported observations from unresolved risks.
- Use no cherry picking to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no cherry picking as permission to invent missing details.
