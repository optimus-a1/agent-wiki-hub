...
# Benchmark Source Gate

## Purpose
This page gives agents a stable rule pattern for benchmark source gate in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Benchmark Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use benchmark source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use benchmark source gate as a checklist item before the agent produces a final answer.
- Use benchmark source gate during review to separate supported observations from unresolved risks.
- Use benchmark source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating benchmark source gate as permission to invent missing details.
