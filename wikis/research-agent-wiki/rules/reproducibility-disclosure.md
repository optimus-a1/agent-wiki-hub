---
title: "Reproducibility Disclosure"
...
## Purpose
This page gives agents a stable rule pattern for reproducibility disclosure in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Reproducibility Disclosure should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use reproducibility disclosure to convert a vague request into explicit fields, checks, and boundaries.
- Use reproducibility disclosure as a checklist item before the agent produces a final answer.
- Use reproducibility disclosure during review to separate supported observations from unresolved risks.
- Use reproducibility disclosure to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating reproducibility disclosure as permission to invent missing details.
