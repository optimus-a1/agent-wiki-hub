...
## Purpose
This page gives agents a stable rule pattern for exact claim support in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Exact Claim Support should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use exact claim support to convert a vague request into explicit fields, checks, and boundaries.
- Use exact claim support as a checklist item before the agent produces a final answer.
- Use exact claim support during review to separate supported observations from unresolved risks.
- Use exact claim support to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating exact claim support as permission to invent missing details.
