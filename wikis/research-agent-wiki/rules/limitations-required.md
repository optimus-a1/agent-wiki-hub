...
## Purpose
This page gives agents a stable rule pattern for limitations required in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Limitations Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use limitations required to convert a vague request into explicit fields, checks, and boundaries.
- Use limitations required as a checklist item before the agent produces a final answer.
- Use limitations required during review to separate supported observations from unresolved risks.
- Use limitations required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating limitations required as permission to invent missing details.
