...
## Purpose
This page gives agents a stable rule pattern for no fabricated citations in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Fabricated Citations should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no fabricated citations to convert a vague request into explicit fields, checks, and boundaries.
- Use no fabricated citations as a checklist item before the agent produces a final answer.
- Use no fabricated citations during review to separate supported observations from unresolved risks.
- Use no fabricated citations to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no fabricated citations as permission to invent missing details.
