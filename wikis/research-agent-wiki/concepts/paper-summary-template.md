...
## Purpose
This page gives agents a stable concept pattern for paper summary template in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Paper Summary Template should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use paper summary template to convert a vague request into explicit fields, checks, and boundaries.
- Use paper summary template as a checklist item before the agent produces a final answer.
- Use paper summary template during review to separate supported observations from unresolved risks.
- Use paper summary template to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating paper summary template as permission to invent missing details.
