...
## Purpose
This page gives agents a stable concept pattern for benchmark interpretation in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Benchmark Interpretation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use benchmark interpretation to convert a vague request into explicit fields, checks, and boundaries.
- Use benchmark interpretation as a checklist item before the agent produces a final answer.
- Use benchmark interpretation during review to separate supported observations from unresolved risks.
- Use benchmark interpretation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating benchmark interpretation as permission to invent missing details.
