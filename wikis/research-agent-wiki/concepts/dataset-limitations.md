...
## Purpose
This page gives agents a stable concept pattern for dataset limitations in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dataset Limitations should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use dataset limitations to convert a vague request into explicit fields, checks, and boundaries.
- Use dataset limitations as a checklist item before the agent produces a final answer.
- Use dataset limitations during review to separate supported observations from unresolved risks.
- Use dataset limitations to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dataset limitations as permission to invent missing details.
