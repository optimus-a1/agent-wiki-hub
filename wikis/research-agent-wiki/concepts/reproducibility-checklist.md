...
## Purpose
This page gives agents a stable concept pattern for reproducibility checklist in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Reproducibility Checklist should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use reproducibility checklist to convert a vague request into explicit fields, checks, and boundaries.
- Use reproducibility checklist as a checklist item before the agent produces a final answer.
- Use reproducibility checklist during review to separate supported observations from unresolved risks.
- Use reproducibility checklist to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating reproducibility checklist as permission to invent missing details.
