...
## Purpose
This page gives agents a stable concept pattern for bias and confounding in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Bias And Confounding should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use bias and confounding to convert a vague request into explicit fields, checks, and boundaries.
- Use bias and confounding as a checklist item before the agent produces a final answer.
- Use bias and confounding during review to separate supported observations from unresolved risks.
- Use bias and confounding to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating bias and confounding as permission to invent missing details.
