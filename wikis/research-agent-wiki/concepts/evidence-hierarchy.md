...
## Purpose
This page gives agents a stable concept pattern for evidence hierarchy in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Evidence Hierarchy should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use evidence hierarchy to convert a vague request into explicit fields, checks, and boundaries.
- Use evidence hierarchy as a checklist item before the agent produces a final answer.
- Use evidence hierarchy during review to separate supported observations from unresolved risks.
- Use evidence hierarchy to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating evidence hierarchy as permission to invent missing details.
