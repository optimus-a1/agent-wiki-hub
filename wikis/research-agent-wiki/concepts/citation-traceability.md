...
## Purpose
This page gives agents a stable concept pattern for citation traceability in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Citation Traceability should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use citation traceability to convert a vague request into explicit fields, checks, and boundaries.
- Use citation traceability as a checklist item before the agent produces a final answer.
- Use citation traceability during review to separate supported observations from unresolved risks.
- Use citation traceability to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating citation traceability as permission to invent missing details.
