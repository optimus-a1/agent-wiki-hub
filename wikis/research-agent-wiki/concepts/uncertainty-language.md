...
## Purpose
This page gives agents a stable concept pattern for uncertainty language in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Uncertainty Language should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use uncertainty language to convert a vague request into explicit fields, checks, and boundaries.
- Use uncertainty language as a checklist item before the agent produces a final answer.
- Use uncertainty language during review to separate supported observations from unresolved risks.
- Use uncertainty language to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating uncertainty language as permission to invent missing details.
