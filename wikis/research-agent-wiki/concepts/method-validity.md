...
## Purpose
This page gives agents a stable concept pattern for method validity in `research-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Method Validity should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use method validity to convert a vague request into explicit fields, checks, and boundaries.
- Use method validity as a checklist item before the agent produces a final answer.
- Use method validity during review to separate supported observations from unresolved risks.
- Use method validity to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating method validity as permission to invent missing details.
