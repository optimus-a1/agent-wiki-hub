...
## Purpose
This page gives agents a stable concept pattern for red flag escalation in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Red Flag Escalation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use red flag escalation to convert a vague request into explicit fields, checks, and boundaries.
- Use red flag escalation as a checklist item before the agent produces a final answer.
- Use red flag escalation during review to separate supported observations from unresolved risks.
- Use red flag escalation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating red flag escalation as permission to invent missing details.
