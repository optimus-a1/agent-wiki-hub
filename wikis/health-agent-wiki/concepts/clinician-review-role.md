...
# Clinician Review Role

## Purpose
This page gives agents a stable concept pattern for clinician review role in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Clinician Review Role should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use clinician review role to convert a vague request into explicit fields, checks, and boundaries.
- Use clinician review role as a checklist item before the agent produces a final answer.
- Use clinician review role during review to separate supported observations from unresolved risks.
- Use clinician review role to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating clinician review role as permission to invent missing details.
