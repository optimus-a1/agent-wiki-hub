---
title: "Case Diagnosis Request Refusal"
...
# Case Diagnosis Request Refusal

## Purpose
This page gives agents a stable case pattern for case diagnosis request refusal in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Diagnosis Request Refusal should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case diagnosis request refusal to convert a vague request into explicit fields, checks, and boundaries.
- Use case diagnosis request refusal as a checklist item before the agent produces a final answer.
- Use case diagnosis request refusal during review to separate supported observations from unresolved risks.
- Use case diagnosis request refusal to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case diagnosis request refusal as permission to invent missing details.
