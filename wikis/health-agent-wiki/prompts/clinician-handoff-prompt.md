---
title: "Clinician Handoff Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for clinician handoff prompt in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Clinician Handoff Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use clinician handoff prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use clinician handoff prompt as a checklist item before the agent produces a final answer.
- Use clinician handoff prompt during review to separate supported observations from unresolved risks.
- Use clinician handoff prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating clinician handoff prompt as permission to invent missing details.
