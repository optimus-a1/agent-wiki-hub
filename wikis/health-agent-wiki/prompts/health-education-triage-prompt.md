---
title: "Health Education Triage Prompt"
...
# Health Education Triage Prompt

## Purpose
This page gives agents a stable prompt pattern for health education triage prompt in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Health Education Triage Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use health education triage prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use health education triage prompt as a checklist item before the agent produces a final answer.
- Use health education triage prompt during review to separate supported observations from unresolved risks.
- Use health education triage prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating health education triage prompt as permission to invent missing details.
