---
title: "Red Flag Screening Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for red flag screening prompt in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Red Flag Screening Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use red flag screening prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use red flag screening prompt as a checklist item before the agent produces a final answer.
- Use red flag screening prompt during review to separate supported observations from unresolved risks.
- Use red flag screening prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating red flag screening prompt as permission to invent missing details.
