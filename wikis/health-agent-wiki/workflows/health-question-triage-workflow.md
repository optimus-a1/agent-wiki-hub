---
title: "Health Question Triage Workflow"
...
# Health Question Triage Workflow

## Purpose
This page gives agents a stable workflow pattern for health question triage workflow in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Health Question Triage Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use health question triage workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use health question triage workflow as a checklist item before the agent produces a final answer.
- Use health question triage workflow during review to separate supported observations from unresolved risks.
- Use health question triage workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating health question triage workflow as permission to invent missing details.
