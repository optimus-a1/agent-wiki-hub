---
title: "Clinician Handoff Summary Workflow"
...
# Clinician Handoff Summary Workflow

## Purpose
This page gives agents a stable workflow pattern for clinician handoff summary workflow in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Clinician Handoff Summary Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use clinician handoff summary workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use clinician handoff summary workflow as a checklist item before the agent produces a final answer.
- Use clinician handoff summary workflow during review to separate supported observations from unresolved risks.
- Use clinician handoff summary workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating clinician handoff summary workflow as permission to invent missing details.
