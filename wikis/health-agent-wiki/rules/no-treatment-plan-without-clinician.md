---
title: "No Treatment Plan Without Clinician"
...
# No Treatment Plan Without Clinician

## Purpose
This page gives agents a stable rule pattern for no treatment plan without clinician in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Treatment Plan Without Clinician should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no treatment plan without clinician to convert a vague request into explicit fields, checks, and boundaries.
- Use no treatment plan without clinician as a checklist item before the agent produces a final answer.
- Use no treatment plan without clinician during review to separate supported observations from unresolved risks.
- Use no treatment plan without clinician to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no treatment plan without clinician as permission to invent missing details.
