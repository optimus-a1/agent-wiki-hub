...
# No Dosage Instruction

## Purpose
This page gives agents a stable rule pattern for no dosage instruction in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Dosage Instruction should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no dosage instruction to convert a vague request into explicit fields, checks, and boundaries.
- Use no dosage instruction as a checklist item before the agent produces a final answer.
- Use no dosage instruction during review to separate supported observations from unresolved risks.
- Use no dosage instruction to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no dosage instruction as permission to invent missing details.
