---
title: "Licensed Clinician Review Required"
...
## Purpose
This page gives agents a stable rule pattern for licensed clinician review required in `health-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Licensed Clinician Review Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use licensed clinician review required to convert a vague request into explicit fields, checks, and boundaries.
- Use licensed clinician review required as a checklist item before the agent produces a final answer.
- Use licensed clinician review required during review to separate supported observations from unresolved risks.
- Use licensed clinician review required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating licensed clinician review required as permission to invent missing details.
