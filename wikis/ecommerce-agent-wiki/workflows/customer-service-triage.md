...
## Purpose
This page gives agents a stable workflow pattern for customer service triage in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Customer Service Triage should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use customer service triage to convert a vague request into explicit fields, checks, and boundaries.
- Use customer service triage as a checklist item before the agent produces a final answer.
- Use customer service triage during review to separate supported observations from unresolved risks.
- Use customer service triage to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating customer service triage as permission to invent missing details.
