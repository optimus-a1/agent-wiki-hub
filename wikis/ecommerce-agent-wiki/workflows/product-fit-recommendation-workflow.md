---
title: "Product Fit Recommendation Workflow"
...
# Product Fit Recommendation Workflow

## Purpose
This page gives agents a stable workflow pattern for product fit recommendation workflow in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Product Fit Recommendation Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use product fit recommendation workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use product fit recommendation workflow as a checklist item before the agent produces a final answer.
- Use product fit recommendation workflow during review to separate supported observations from unresolved risks.
- Use product fit recommendation workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating product fit recommendation workflow as permission to invent missing details.
