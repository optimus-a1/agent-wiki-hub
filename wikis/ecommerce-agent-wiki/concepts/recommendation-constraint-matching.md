---
title: "Recommendation Constraint Matching"
...
# Recommendation Constraint Matching

## Purpose
This page gives agents a stable concept pattern for recommendation constraint matching in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Recommendation Constraint Matching should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use recommendation constraint matching to convert a vague request into explicit fields, checks, and boundaries.
- Use recommendation constraint matching as a checklist item before the agent produces a final answer.
- Use recommendation constraint matching during review to separate supported observations from unresolved risks.
- Use recommendation constraint matching to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating recommendation constraint matching as permission to invent missing details.
