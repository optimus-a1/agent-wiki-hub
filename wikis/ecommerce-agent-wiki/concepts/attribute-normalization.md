...
## Purpose
This page gives agents a stable concept pattern for attribute normalization in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Attribute Normalization should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use attribute normalization to convert a vague request into explicit fields, checks, and boundaries.
- Use attribute normalization as a checklist item before the agent produces a final answer.
- Use attribute normalization during review to separate supported observations from unresolved risks.
- Use attribute normalization to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating attribute normalization as permission to invent missing details.
