...
## Purpose
This page gives agents a stable concept pattern for product catalog model in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Product Catalog Model should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use product catalog model to convert a vague request into explicit fields, checks, and boundaries.
- Use product catalog model as a checklist item before the agent produces a final answer.
- Use product catalog model during review to separate supported observations from unresolved risks.
- Use product catalog model to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating product catalog model as permission to invent missing details.
