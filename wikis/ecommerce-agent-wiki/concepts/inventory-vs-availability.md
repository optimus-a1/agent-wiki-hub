---
title: "Inventory Vs Availability"
...
## Purpose
This page gives agents a stable concept pattern for inventory vs availability in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Inventory Vs Availability should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use inventory vs availability to convert a vague request into explicit fields, checks, and boundaries.
- Use inventory vs availability as a checklist item before the agent produces a final answer.
- Use inventory vs availability during review to separate supported observations from unresolved risks.
- Use inventory vs availability to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating inventory vs availability as permission to invent missing details.
