...
## Purpose
This page gives agents a stable concept pattern for customer intent signals in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Customer Intent Signals should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use customer intent signals to convert a vague request into explicit fields, checks, and boundaries.
- Use customer intent signals as a checklist item before the agent produces a final answer.
- Use customer intent signals during review to separate supported observations from unresolved risks.
- Use customer intent signals to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating customer intent signals as permission to invent missing details.
