---
title: "Platform Policy Source Gate Prompt"
...
# Platform Policy Source Gate Prompt

## Purpose
This page gives agents a stable prompt pattern for platform policy source gate prompt in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Platform Policy Source Gate Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use platform policy source gate prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use platform policy source gate prompt as a checklist item before the agent produces a final answer.
- Use platform policy source gate prompt during review to separate supported observations from unresolved risks.
- Use platform policy source gate prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating platform policy source gate prompt as permission to invent missing details.
