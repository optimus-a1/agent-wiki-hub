---
title: "Platform Policy Source Gate"
...
# Platform Policy Source Gate

## Purpose
This page gives agents a stable rule pattern for platform policy source gate in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Platform Policy Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use platform policy source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use platform policy source gate as a checklist item before the agent produces a final answer.
- Use platform policy source gate during review to separate supported observations from unresolved risks.
- Use platform policy source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating platform policy source gate as permission to invent missing details.
