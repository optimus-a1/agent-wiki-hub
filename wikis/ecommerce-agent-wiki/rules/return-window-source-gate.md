---
title: "Return Window Source Gate"
...
# Return Window Source Gate

## Purpose
This page gives agents a stable rule pattern for return window source gate in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Return Window Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use return window source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use return window source gate as a checklist item before the agent produces a final answer.
- Use return window source gate during review to separate supported observations from unresolved risks.
- Use return window source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating return window source gate as permission to invent missing details.
