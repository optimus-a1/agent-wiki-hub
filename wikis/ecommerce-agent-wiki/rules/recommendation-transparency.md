...
## Purpose
This page gives agents a stable rule pattern for recommendation transparency in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Recommendation Transparency should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use recommendation transparency to convert a vague request into explicit fields, checks, and boundaries.
- Use recommendation transparency as a checklist item before the agent produces a final answer.
- Use recommendation transparency during review to separate supported observations from unresolved risks.
- Use recommendation transparency to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating recommendation transparency as permission to invent missing details.
