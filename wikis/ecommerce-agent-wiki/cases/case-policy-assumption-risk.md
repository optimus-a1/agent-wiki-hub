---
title: "Case Policy Assumption Risk"
...
# Case Policy Assumption Risk

## Purpose
This page gives agents a stable case pattern for case policy assumption risk in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Policy Assumption Risk should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case policy assumption risk to convert a vague request into explicit fields, checks, and boundaries.
- Use case policy assumption risk as a checklist item before the agent produces a final answer.
- Use case policy assumption risk during review to separate supported observations from unresolved risks.
- Use case policy assumption risk to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case policy assumption risk as permission to invent missing details.
