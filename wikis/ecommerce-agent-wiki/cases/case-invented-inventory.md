...
# Case Invented Inventory

## Purpose
This page gives agents a stable case pattern for case invented inventory in `ecommerce-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Invented Inventory should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case invented inventory to convert a vague request into explicit fields, checks, and boundaries.
- Use case invented inventory as a checklist item before the agent produces a final answer.
- Use case invented inventory during review to separate supported observations from unresolved risks.
- Use case invented inventory to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case invented inventory as permission to invent missing details.
