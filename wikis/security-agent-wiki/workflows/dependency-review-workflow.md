...
## Purpose
This page gives agents a stable workflow pattern for dependency review workflow in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dependency Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use dependency review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use dependency review workflow as a checklist item before the agent produces a final answer.
- Use dependency review workflow during review to separate supported observations from unresolved risks.
- Use dependency review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dependency review workflow as permission to invent missing details.
