---
title: "Incident Containment Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for incident containment workflow in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Incident Containment Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use incident containment workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use incident containment workflow as a checklist item before the agent produces a final answer.
- Use incident containment workflow during review to separate supported observations from unresolved risks.
- Use incident containment workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating incident containment workflow as permission to invent missing details.
