---
title: "Detection Rule Review Workflow"
...
# Detection Rule Review Workflow

## Purpose
This page gives agents a stable workflow pattern for detection rule review workflow in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Detection Rule Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use detection rule review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use detection rule review workflow as a checklist item before the agent produces a final answer.
- Use detection rule review workflow during review to separate supported observations from unresolved risks.
- Use detection rule review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating detection rule review workflow as permission to invent missing details.
