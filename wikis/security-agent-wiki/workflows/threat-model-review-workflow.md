---
title: "Threat Model Review Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for threat model review workflow in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Threat Model Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use threat model review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use threat model review workflow as a checklist item before the agent produces a final answer.
- Use threat model review workflow during review to separate supported observations from unresolved risks.
- Use threat model review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating threat model review workflow as permission to invent missing details.
