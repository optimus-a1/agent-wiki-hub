---
title: "Secret Exposure Response Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for secret exposure response workflow in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Secret Exposure Response Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use secret exposure response workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use secret exposure response workflow as a checklist item before the agent produces a final answer.
- Use secret exposure response workflow during review to separate supported observations from unresolved risks.
- Use secret exposure response workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating secret exposure response workflow as permission to invent missing details.
