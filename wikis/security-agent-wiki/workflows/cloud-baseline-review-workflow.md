---
title: "Cloud Baseline Review Workflow"
...
# Cloud Baseline Review Workflow

## Purpose
This page gives agents a stable workflow pattern for cloud baseline review workflow in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Cloud Baseline Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use cloud baseline review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use cloud baseline review workflow as a checklist item before the agent produces a final answer.
- Use cloud baseline review workflow during review to separate supported observations from unresolved risks.
- Use cloud baseline review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating cloud baseline review workflow as permission to invent missing details.
