---
title: "Case Log Redaction Success"
...
# Case Log Redaction Success

## Purpose
This page gives agents a stable case pattern for case log redaction success in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Log Redaction Success should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case log redaction success to convert a vague request into explicit fields, checks, and boundaries.
- Use case log redaction success as a checklist item before the agent produces a final answer.
- Use case log redaction success during review to separate supported observations from unresolved risks.
- Use case log redaction success to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case log redaction success as permission to invent missing details.
