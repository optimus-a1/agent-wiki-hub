---
title: "Case Running Unknown Script"
...
# Case Running Unknown Script

## Purpose
This page gives agents a stable case pattern for case running unknown script in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Running Unknown Script should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case running unknown script to convert a vague request into explicit fields, checks, and boundaries.
- Use case running unknown script as a checklist item before the agent produces a final answer.
- Use case running unknown script during review to separate supported observations from unresolved risks.
- Use case running unknown script to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case running unknown script as permission to invent missing details.
