...
## Purpose
This page gives agents a stable concept pattern for asset exposure model in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Asset Exposure Model should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use asset exposure model to convert a vague request into explicit fields, checks, and boundaries.
- Use asset exposure model as a checklist item before the agent produces a final answer.
- Use asset exposure model during review to separate supported observations from unresolved risks.
- Use asset exposure model to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating asset exposure model as permission to invent missing details.
