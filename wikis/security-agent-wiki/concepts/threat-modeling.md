...
## Purpose
This page gives agents a stable concept pattern for threat modeling in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Threat Modeling should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use threat modeling to convert a vague request into explicit fields, checks, and boundaries.
- Use threat modeling as a checklist item before the agent produces a final answer.
- Use threat modeling during review to separate supported observations from unresolved risks.
- Use threat modeling to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating threat modeling as permission to invent missing details.
