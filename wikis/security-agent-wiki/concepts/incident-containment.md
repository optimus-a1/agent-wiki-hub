...
## Purpose
This page gives agents a stable concept pattern for incident containment in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Incident Containment should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use incident containment to convert a vague request into explicit fields, checks, and boundaries.
- Use incident containment as a checklist item before the agent produces a final answer.
- Use incident containment during review to separate supported observations from unresolved risks.
- Use incident containment to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating incident containment as permission to invent missing details.
