...
## Purpose
This page gives agents a stable concept pattern for least privilege in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Least Privilege should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use least privilege to convert a vague request into explicit fields, checks, and boundaries.
- Use least privilege as a checklist item before the agent produces a final answer.
- Use least privilege during review to separate supported observations from unresolved risks.
- Use least privilege to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating least privilege as permission to invent missing details.
