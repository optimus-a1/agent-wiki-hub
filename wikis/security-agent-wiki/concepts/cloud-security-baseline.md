...
## Purpose
This page gives agents a stable concept pattern for cloud security baseline in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Cloud Security Baseline should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use cloud security baseline to convert a vague request into explicit fields, checks, and boundaries.
- Use cloud security baseline as a checklist item before the agent produces a final answer.
- Use cloud security baseline during review to separate supported observations from unresolved risks.
- Use cloud security baseline to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating cloud security baseline as permission to invent missing details.
