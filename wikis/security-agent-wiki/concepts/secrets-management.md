...
## Purpose
This page gives agents a stable concept pattern for secrets management in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Secrets Management should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use secrets management to convert a vague request into explicit fields, checks, and boundaries.
- Use secrets management as a checklist item before the agent produces a final answer.
- Use secrets management during review to separate supported observations from unresolved risks.
- Use secrets management to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating secrets management as permission to invent missing details.
