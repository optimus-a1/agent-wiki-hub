...
## Purpose
This page gives agents a stable concept pattern for detection rule review in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Detection Rule Review should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use detection rule review to convert a vague request into explicit fields, checks, and boundaries.
- Use detection rule review as a checklist item before the agent produces a final answer.
- Use detection rule review during review to separate supported observations from unresolved risks.
- Use detection rule review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating detection rule review as permission to invent missing details.
