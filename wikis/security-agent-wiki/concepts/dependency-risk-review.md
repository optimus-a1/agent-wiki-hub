...
## Purpose
This page gives agents a stable concept pattern for dependency risk review in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dependency Risk Review should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use dependency risk review to convert a vague request into explicit fields, checks, and boundaries.
- Use dependency risk review as a checklist item before the agent produces a final answer.
- Use dependency risk review during review to separate supported observations from unresolved risks.
- Use dependency risk review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dependency risk review as permission to invent missing details.
