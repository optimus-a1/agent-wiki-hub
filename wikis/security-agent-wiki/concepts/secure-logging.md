...
## Purpose
This page gives agents a stable concept pattern for secure logging in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Secure Logging should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use secure logging to convert a vague request into explicit fields, checks, and boundaries.
- Use secure logging as a checklist item before the agent produces a final answer.
- Use secure logging during review to separate supported observations from unresolved risks.
- Use secure logging to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating secure logging as permission to invent missing details.
