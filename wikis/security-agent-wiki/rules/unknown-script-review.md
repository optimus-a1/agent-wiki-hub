...
# Unknown Script Review

## Purpose
This page gives agents a stable rule pattern for unknown script review in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Unknown Script Review should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use unknown script review to convert a vague request into explicit fields, checks, and boundaries.
- Use unknown script review as a checklist item before the agent produces a final answer.
- Use unknown script review during review to separate supported observations from unresolved risks.
- Use unknown script review to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating unknown script review as permission to invent missing details.
