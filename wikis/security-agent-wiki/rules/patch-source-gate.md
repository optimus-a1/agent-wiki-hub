...
## Purpose
This page gives agents a stable rule pattern for patch source gate in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Patch Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use patch source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use patch source gate as a checklist item before the agent produces a final answer.
- Use patch source gate during review to separate supported observations from unresolved risks.
- Use patch source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating patch source gate as permission to invent missing details.
