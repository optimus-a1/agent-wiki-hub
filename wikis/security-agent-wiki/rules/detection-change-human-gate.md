---
title: "Detection Change Human Gate"
...
# Detection Change Human Gate

## Purpose
This page gives agents a stable rule pattern for detection change human gate in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Detection Change Human Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use detection change human gate to convert a vague request into explicit fields, checks, and boundaries.
- Use detection change human gate as a checklist item before the agent produces a final answer.
- Use detection change human gate during review to separate supported observations from unresolved risks.
- Use detection change human gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating detection change human gate as permission to invent missing details.
