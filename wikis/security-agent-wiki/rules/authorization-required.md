...
## Purpose
This page gives agents a stable rule pattern for authorization required in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Authorization Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use authorization required to convert a vague request into explicit fields, checks, and boundaries.
- Use authorization required as a checklist item before the agent produces a final answer.
- Use authorization required during review to separate supported observations from unresolved risks.
- Use authorization required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating authorization required as permission to invent missing details.
