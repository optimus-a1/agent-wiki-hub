...
## Purpose
This page gives agents a stable rule pattern for no credential theft in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Credential Theft should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no credential theft to convert a vague request into explicit fields, checks, and boundaries.
- Use no credential theft as a checklist item before the agent produces a final answer.
- Use no credential theft during review to separate supported observations from unresolved risks.
- Use no credential theft to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no credential theft as permission to invent missing details.
