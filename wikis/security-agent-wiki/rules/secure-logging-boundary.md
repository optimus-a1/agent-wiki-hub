...
# Secure Logging Boundary

## Purpose
This page gives agents a stable rule pattern for secure logging boundary in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Secure Logging Boundary should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use secure logging boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use secure logging boundary as a checklist item before the agent produces a final answer.
- Use secure logging boundary during review to separate supported observations from unresolved risks.
- Use secure logging boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating secure logging boundary as permission to invent missing details.
