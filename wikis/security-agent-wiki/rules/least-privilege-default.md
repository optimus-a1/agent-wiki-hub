...
## Purpose
This page gives agents a stable rule pattern for least privilege default in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Least Privilege Default should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use least privilege default to convert a vague request into explicit fields, checks, and boundaries.
- Use least privilege default as a checklist item before the agent produces a final answer.
- Use least privilege default during review to separate supported observations from unresolved risks.
- Use least privilege default to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating least privilege default as permission to invent missing details.
