...
## Purpose
This page gives agents a stable rule pattern for defensive only boundary in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Defensive Only Boundary should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use defensive only boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use defensive only boundary as a checklist item before the agent produces a final answer.
- Use defensive only boundary during review to separate supported observations from unresolved risks.
- Use defensive only boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating defensive only boundary as permission to invent missing details.
