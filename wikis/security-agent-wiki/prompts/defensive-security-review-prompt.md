---
title: "Defensive Security Review Prompt"
...
# Defensive Security Review Prompt

## Purpose
This page gives agents a stable prompt pattern for defensive security review prompt in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Defensive Security Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use defensive security review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use defensive security review prompt as a checklist item before the agent produces a final answer.
- Use defensive security review prompt during review to separate supported observations from unresolved risks.
- Use defensive security review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating defensive security review prompt as permission to invent missing details.
