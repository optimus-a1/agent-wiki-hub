---
title: "Dependency Security Review Prompt"
...
# Dependency Security Review Prompt

## Purpose
This page gives agents a stable prompt pattern for dependency security review prompt in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dependency Security Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Human Gate
Require human confirmation for remediation that changes production access, secrets, logging, detection coverage, or customer-facing security posture.

## Source Gate
CVE status, vendor advisories, exploitability, patches, detection signatures, and tool rules require source review before use.

## Examples
- Use dependency security review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use dependency security review prompt as a checklist item before the agent produces a final answer.
- Use dependency security review prompt during review to separate supported observations from unresolved risks.
- Use dependency security review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dependency security review prompt as permission to invent missing details.
