---
title: "Secret Leak Triage Prompt"
...
# Secret Leak Triage Prompt

## Purpose
This page gives agents a stable prompt pattern for secret leak triage prompt in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Secret Leak Triage Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use secret leak triage prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use secret leak triage prompt as a checklist item before the agent produces a final answer.
- Use secret leak triage prompt during review to separate supported observations from unresolved risks.
- Use secret leak triage prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating secret leak triage prompt as permission to invent missing details.
