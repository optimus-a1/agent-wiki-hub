...
## Purpose
This page gives agents a stable prompt pattern for threat model prompt in `security-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Threat Model Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use threat model prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use threat model prompt as a checklist item before the agent produces a final answer.
- Use threat model prompt during review to separate supported observations from unresolved risks.
- Use threat model prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating threat model prompt as permission to invent missing details.
