...
## Purpose
This page gives agents a stable workflow pattern for prompt routing workflow in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Prompt Routing Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use prompt routing workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use prompt routing workflow as a checklist item before the agent produces a final answer.
- Use prompt routing workflow during review to separate supported observations from unresolved risks.
- Use prompt routing workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating prompt routing workflow as permission to invent missing details.
