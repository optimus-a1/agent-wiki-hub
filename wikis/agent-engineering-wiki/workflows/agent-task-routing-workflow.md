---
title: "Agent Task Routing Workflow"
...
# Agent Task Routing Workflow

## Purpose
This page gives agents a stable workflow pattern for agent task routing workflow in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Agent Task Routing Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use agent task routing workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use agent task routing workflow as a checklist item before the agent produces a final answer.
- Use agent task routing workflow during review to separate supported observations from unresolved risks.
- Use agent task routing workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating agent task routing workflow as permission to invent missing details.
