...
## Purpose
This page gives agents a stable workflow pattern for memory review workflow in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Memory Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use memory review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use memory review workflow as a checklist item before the agent produces a final answer.
- Use memory review workflow during review to separate supported observations from unresolved risks.
- Use memory review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating memory review workflow as permission to invent missing details.
