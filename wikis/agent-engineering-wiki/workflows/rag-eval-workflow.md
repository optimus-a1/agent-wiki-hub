...
## Purpose
This page gives agents a stable workflow pattern for rag eval workflow in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- RAG Eval Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use rag eval workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use rag eval workflow as a checklist item before the agent produces a final answer.
- Use rag eval workflow during review to separate supported observations from unresolved risks.
- Use rag eval workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating rag eval workflow as permission to invent missing details.
