...
# Case RAG Citation Gap

## Purpose
This page gives agents a stable case pattern for case rag citation gap in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case RAG Citation Gap should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case rag citation gap to convert a vague request into explicit fields, checks, and boundaries.
- Use case rag citation gap as a checklist item before the agent produces a final answer.
- Use case rag citation gap during review to separate supported observations from unresolved risks.
- Use case rag citation gap to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case rag citation gap as permission to invent missing details.
