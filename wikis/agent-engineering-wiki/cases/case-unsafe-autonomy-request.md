---
title: "Case Unsafe Autonomy Request"
...
# Case Unsafe Autonomy Request

## Purpose
This page gives agents a stable case pattern for case unsafe autonomy request in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Unsafe Autonomy Request should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case unsafe autonomy request to convert a vague request into explicit fields, checks, and boundaries.
- Use case unsafe autonomy request as a checklist item before the agent produces a final answer.
- Use case unsafe autonomy request during review to separate supported observations from unresolved risks.
- Use case unsafe autonomy request to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case unsafe autonomy request as permission to invent missing details.
