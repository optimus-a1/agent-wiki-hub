---
title: "Case Ungrounded Agent Answer"
...
# Case Ungrounded Agent Answer

## Purpose
This page gives agents a stable case pattern for case ungrounded agent answer in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Ungrounded Agent Answer should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case ungrounded agent answer to convert a vague request into explicit fields, checks, and boundaries.
- Use case ungrounded agent answer as a checklist item before the agent produces a final answer.
- Use case ungrounded agent answer during review to separate supported observations from unresolved risks.
- Use case ungrounded agent answer to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case ungrounded agent answer as permission to invent missing details.
