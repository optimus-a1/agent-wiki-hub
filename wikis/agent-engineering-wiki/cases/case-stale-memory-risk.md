...
# Case Stale Memory Risk

## Purpose
This page gives agents a stable case pattern for case stale memory risk in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Stale Memory Risk should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case stale memory risk to convert a vague request into explicit fields, checks, and boundaries.
- Use case stale memory risk as a checklist item before the agent produces a final answer.
- Use case stale memory risk during review to separate supported observations from unresolved risks.
- Use case stale memory risk to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case stale memory risk as permission to invent missing details.
