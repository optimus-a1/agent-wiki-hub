...
## Purpose
This page gives agents a stable prompt pattern for eval design prompt in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Eval Design Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use eval design prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use eval design prompt as a checklist item before the agent produces a final answer.
- Use eval design prompt during review to separate supported observations from unresolved risks.
- Use eval design prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating eval design prompt as permission to invent missing details.
