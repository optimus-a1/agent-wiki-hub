---
title: "RAG Grounding Review Prompt"
...
# RAG Grounding Review Prompt

## Purpose
This page gives agents a stable prompt pattern for rag grounding review prompt in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- RAG Grounding Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use rag grounding review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use rag grounding review prompt as a checklist item before the agent produces a final answer.
- Use rag grounding review prompt during review to separate supported observations from unresolved risks.
- Use rag grounding review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating rag grounding review prompt as permission to invent missing details.
