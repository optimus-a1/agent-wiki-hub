...
## Purpose
This page gives agents a stable prompt pattern for agent routing prompt in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Agent Routing Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use agent routing prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use agent routing prompt as a checklist item before the agent produces a final answer.
- Use agent routing prompt during review to separate supported observations from unresolved risks.
- Use agent routing prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating agent routing prompt as permission to invent missing details.
