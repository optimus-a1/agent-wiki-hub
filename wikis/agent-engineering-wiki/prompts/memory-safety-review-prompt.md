---
title: "Memory Safety Review Prompt"
...
# Memory Safety Review Prompt

## Purpose
This page gives agents a stable prompt pattern for memory safety review prompt in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Memory Safety Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use memory safety review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use memory safety review prompt as a checklist item before the agent produces a final answer.
- Use memory safety review prompt during review to separate supported observations from unresolved risks.
- Use memory safety review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating memory safety review prompt as permission to invent missing details.
