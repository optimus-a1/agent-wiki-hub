...
## Purpose
This page gives agents a stable concept pattern for agent planning loop in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Agent Planning Loop should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use agent planning loop to convert a vague request into explicit fields, checks, and boundaries.
- Use agent planning loop as a checklist item before the agent produces a final answer.
- Use agent planning loop during review to separate supported observations from unresolved risks.
- Use agent planning loop to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating agent planning loop as permission to invent missing details.
