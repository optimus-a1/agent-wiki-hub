...
## Purpose
This page gives agents a stable concept pattern for tool calling contract in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Tool Calling Contract should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use tool calling contract to convert a vague request into explicit fields, checks, and boundaries.
- Use tool calling contract as a checklist item before the agent produces a final answer.
- Use tool calling contract during review to separate supported observations from unresolved risks.
- Use tool calling contract to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating tool calling contract as permission to invent missing details.
