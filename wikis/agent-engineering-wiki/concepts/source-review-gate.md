...
## Purpose
This page gives agents a stable concept pattern for source review gate in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Source Review Gate should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use source review gate to convert a vague request into explicit fields, checks, and boundaries.
- Use source review gate as a checklist item before the agent produces a final answer.
- Use source review gate during review to separate supported observations from unresolved risks.
- Use source review gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating source review gate as permission to invent missing details.
