...
## Purpose
This page gives agents a stable concept pattern for agent system components in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Agent System Components should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use agent system components to convert a vague request into explicit fields, checks, and boundaries.
- Use agent system components as a checklist item before the agent produces a final answer.
- Use agent system components during review to separate supported observations from unresolved risks.
- Use agent system components to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating agent system components as permission to invent missing details.
