...
## Purpose
This page gives agents a stable concept pattern for rag retrieval design in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- RAG Retrieval Design should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use rag retrieval design to convert a vague request into explicit fields, checks, and boundaries.
- Use rag retrieval design as a checklist item before the agent produces a final answer.
- Use rag retrieval design during review to separate supported observations from unresolved risks.
- Use rag retrieval design to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating rag retrieval design as permission to invent missing details.
