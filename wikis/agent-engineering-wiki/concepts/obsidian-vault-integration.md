...
## Purpose
This page gives agents a stable concept pattern for obsidian vault integration in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Obsidian Vault Integration should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use obsidian vault integration to convert a vague request into explicit fields, checks, and boundaries.
- Use obsidian vault integration as a checklist item before the agent produces a final answer.
- Use obsidian vault integration during review to separate supported observations from unresolved risks.
- Use obsidian vault integration to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating obsidian vault integration as permission to invent missing details.
