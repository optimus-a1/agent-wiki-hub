...
## Purpose
This page gives agents a stable concept pattern for prompt routing in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Prompt Routing should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use prompt routing to convert a vague request into explicit fields, checks, and boundaries.
- Use prompt routing as a checklist item before the agent produces a final answer.
- Use prompt routing during review to separate supported observations from unresolved risks.
- Use prompt routing to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating prompt routing as permission to invent missing details.
