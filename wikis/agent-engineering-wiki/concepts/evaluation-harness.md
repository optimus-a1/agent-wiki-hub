...
## Purpose
This page gives agents a stable concept pattern for evaluation harness in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Evaluation Harness should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use evaluation harness to convert a vague request into explicit fields, checks, and boundaries.
- Use evaluation harness as a checklist item before the agent produces a final answer.
- Use evaluation harness during review to separate supported observations from unresolved risks.
- Use evaluation harness to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating evaluation harness as permission to invent missing details.
