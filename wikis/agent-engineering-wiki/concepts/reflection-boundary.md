...
## Purpose
This page gives agents a stable concept pattern for reflection boundary in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Reflection Boundary should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use reflection boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use reflection boundary as a checklist item before the agent produces a final answer.
- Use reflection boundary during review to separate supported observations from unresolved risks.
- Use reflection boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating reflection boundary as permission to invent missing details.
