...
## Purpose
This page gives agents a stable concept pattern for memory hierarchy in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Memory Hierarchy should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use memory hierarchy to convert a vague request into explicit fields, checks, and boundaries.
- Use memory hierarchy as a checklist item before the agent produces a final answer.
- Use memory hierarchy during review to separate supported observations from unresolved risks.
- Use memory hierarchy to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating memory hierarchy as permission to invent missing details.
