...
## Purpose
This page gives agents a stable rule pattern for tool use justification in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Tool Use Justification should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use tool use justification to convert a vague request into explicit fields, checks, and boundaries.
- Use tool use justification as a checklist item before the agent produces a final answer.
- Use tool use justification during review to separate supported observations from unresolved risks.
- Use tool use justification to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating tool use justification as permission to invent missing details.
