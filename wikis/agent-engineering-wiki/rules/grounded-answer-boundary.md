...
## Purpose
This page gives agents a stable rule pattern for grounded answer boundary in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Grounded Answer Boundary should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use grounded answer boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use grounded answer boundary as a checklist item before the agent produces a final answer.
- Use grounded answer boundary during review to separate supported observations from unresolved risks.
- Use grounded answer boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating grounded answer boundary as permission to invent missing details.
