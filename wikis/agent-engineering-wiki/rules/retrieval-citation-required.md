---
title: "Retrieval Citation Required"
...
# Retrieval Citation Required

## Purpose
This page gives agents a stable rule pattern for retrieval citation required in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Retrieval Citation Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use retrieval citation required to convert a vague request into explicit fields, checks, and boundaries.
- Use retrieval citation required as a checklist item before the agent produces a final answer.
- Use retrieval citation required during review to separate supported observations from unresolved risks.
- Use retrieval citation required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating retrieval citation required as permission to invent missing details.
