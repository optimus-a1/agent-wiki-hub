...
# Memory Write Human Gate

## Purpose
This page gives agents a stable rule pattern for memory write human gate in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Memory Write Human Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use memory write human gate to convert a vague request into explicit fields, checks, and boundaries.
- Use memory write human gate as a checklist item before the agent produces a final answer.
- Use memory write human gate during review to separate supported observations from unresolved risks.
- Use memory write human gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating memory write human gate as permission to invent missing details.
