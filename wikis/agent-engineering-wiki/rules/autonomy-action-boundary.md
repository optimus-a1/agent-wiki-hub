---
title: "Autonomy Action Boundary"
...
## Purpose
This page gives agents a stable rule pattern for autonomy action boundary in `agent-engineering-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Autonomy Action Boundary should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use autonomy action boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use autonomy action boundary as a checklist item before the agent produces a final answer.
- Use autonomy action boundary during review to separate supported observations from unresolved risks.
- Use autonomy action boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating autonomy action boundary as permission to invent missing details.
