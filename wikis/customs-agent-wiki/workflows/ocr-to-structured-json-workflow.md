...
## Purpose
This page gives agents a stable workflow pattern for ocr to structured json workflow in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- OCR To Structured JSON Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use ocr to structured json workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use ocr to structured json workflow as a checklist item before the agent produces a final answer.
- Use ocr to structured json workflow during review to separate supported observations from unresolved risks.
- Use ocr to structured json workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating ocr to structured json workflow as permission to invent missing details.
