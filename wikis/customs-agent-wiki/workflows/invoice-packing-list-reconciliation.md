---
title: "Invoice Packing List Reconciliation"
...
# Invoice Packing List Reconciliation

## Purpose
This page gives agents a stable workflow pattern for invoice packing list reconciliation in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Invoice Packing List Reconciliation should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Human Gate
Require human confirmation before filing, declaring, classifying goods, accepting a high-risk discrepancy, or replacing a missing document field.

## Source Gate
HS codes, tax rates, supervision conditions, port rules, customs policy, and regulatory requirements require source review before use.

## Examples
- Use invoice packing list reconciliation to convert a vague request into explicit fields, checks, and boundaries.
- Use invoice packing list reconciliation as a checklist item before the agent produces a final answer.
- Use invoice packing list reconciliation during review to separate supported observations from unresolved risks.
- Use invoice packing list reconciliation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating invoice packing list reconciliation as permission to invent missing details.
