---
title: "Case Weight Inconsistency"
...
# Case Weight Inconsistency

## Purpose
This page gives agents a stable case pattern for case weight inconsistency in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Weight Inconsistency should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case weight inconsistency to convert a vague request into explicit fields, checks, and boundaries.
- Use case weight inconsistency as a checklist item before the agent produces a final answer.
- Use case weight inconsistency during review to separate supported observations from unresolved risks.
- Use case weight inconsistency to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case weight inconsistency as permission to invent missing details.
