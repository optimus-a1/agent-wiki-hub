...
## Purpose
This page gives agents a stable case pattern for case currency mismatch in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Currency Mismatch should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case currency mismatch to convert a vague request into explicit fields, checks, and boundaries.
- Use case currency mismatch as a checklist item before the agent produces a final answer.
- Use case currency mismatch during review to separate supported observations from unresolved risks.
- Use case currency mismatch to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case currency mismatch as permission to invent missing details.
