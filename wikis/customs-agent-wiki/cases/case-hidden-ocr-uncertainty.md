---
title: "Case Hidden OCR Uncertainty"
...
# Case Hidden OCR Uncertainty

## Purpose
This page gives agents a stable case pattern for case hidden ocr uncertainty in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Hidden OCR Uncertainty should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case hidden ocr uncertainty to convert a vague request into explicit fields, checks, and boundaries.
- Use case hidden ocr uncertainty as a checklist item before the agent produces a final answer.
- Use case hidden ocr uncertainty during review to separate supported observations from unresolved risks.
- Use case hidden ocr uncertainty to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case hidden ocr uncertainty as permission to invent missing details.
