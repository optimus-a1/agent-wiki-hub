---
title: "Case Package Count Mismatch"
...
# Case Package Count Mismatch

## Purpose
This page gives agents a stable case pattern for case package count mismatch in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Package Count Mismatch should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case package count mismatch to convert a vague request into explicit fields, checks, and boundaries.
- Use case package count mismatch as a checklist item before the agent produces a final answer.
- Use case package count mismatch during review to separate supported observations from unresolved risks.
- Use case package count mismatch to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case package count mismatch as permission to invent missing details.
