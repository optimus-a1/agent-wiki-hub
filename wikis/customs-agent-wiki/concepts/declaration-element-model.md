---
title: "Declaration Element Model"
...
## Purpose
This page gives agents a stable concept pattern for declaration element model in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Declaration Element Model should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use declaration element model to convert a vague request into explicit fields, checks, and boundaries.
- Use declaration element model as a checklist item before the agent produces a final answer.
- Use declaration element model during review to separate supported observations from unresolved risks.
- Use declaration element model to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating declaration element model as permission to invent missing details.
