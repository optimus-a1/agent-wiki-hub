...
## Purpose
This page gives agents a stable concept pattern for ocr field confidence in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- OCR Field Confidence should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use ocr field confidence to convert a vague request into explicit fields, checks, and boundaries.
- Use ocr field confidence as a checklist item before the agent produces a final answer.
- Use ocr field confidence during review to separate supported observations from unresolved risks.
- Use ocr field confidence to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating ocr field confidence as permission to invent missing details.
