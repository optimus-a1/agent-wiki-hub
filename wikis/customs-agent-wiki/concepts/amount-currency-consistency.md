---
title: "Amount Currency Consistency"
...
## Purpose
This page gives agents a stable concept pattern for amount currency consistency in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Amount Currency Consistency should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use amount currency consistency to convert a vague request into explicit fields, checks, and boundaries.
- Use amount currency consistency as a checklist item before the agent produces a final answer.
- Use amount currency consistency during review to separate supported observations from unresolved risks.
- Use amount currency consistency to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating amount currency consistency as permission to invent missing details.
