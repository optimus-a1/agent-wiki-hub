---
title: "Package Count Reconciliation"
...
## Purpose
This page gives agents a stable concept pattern for package count reconciliation in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Package Count Reconciliation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use package count reconciliation to convert a vague request into explicit fields, checks, and boundaries.
- Use package count reconciliation as a checklist item before the agent produces a final answer.
- Use package count reconciliation during review to separate supported observations from unresolved risks.
- Use package count reconciliation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating package count reconciliation as permission to invent missing details.
