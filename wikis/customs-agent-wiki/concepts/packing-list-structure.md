...
## Purpose
This page gives agents a stable concept pattern for packing list structure in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Packing List Structure should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use packing list structure to convert a vague request into explicit fields, checks, and boundaries.
- Use packing list structure as a checklist item before the agent produces a final answer.
- Use packing list structure during review to separate supported observations from unresolved risks.
- Use packing list structure to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating packing list structure as permission to invent missing details.
