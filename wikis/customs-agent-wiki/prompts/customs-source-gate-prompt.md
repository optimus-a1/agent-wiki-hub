---
title: "Customs Source Gate Prompt"
...
# Customs Source Gate Prompt

## Purpose
This page gives agents a stable prompt pattern for customs source gate prompt in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Customs Source Gate Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use customs source gate prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use customs source gate prompt as a checklist item before the agent produces a final answer.
- Use customs source gate prompt during review to separate supported observations from unresolved risks.
- Use customs source gate prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating customs source gate prompt as permission to invent missing details.
