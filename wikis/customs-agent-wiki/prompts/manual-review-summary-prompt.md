---
title: "Manual Review Summary Prompt"
...
# Manual Review Summary Prompt

## Purpose
This page gives agents a stable prompt pattern for manual review summary prompt in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Manual Review Summary Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use manual review summary prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use manual review summary prompt as a checklist item before the agent produces a final answer.
- Use manual review summary prompt during review to separate supported observations from unresolved risks.
- Use manual review summary prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating manual review summary prompt as permission to invent missing details.
