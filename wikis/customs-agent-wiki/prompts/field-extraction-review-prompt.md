---
title: "Field Extraction Review Prompt"
...
# Field Extraction Review Prompt

## Purpose
This page gives agents a stable prompt pattern for field extraction review prompt in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Field Extraction Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use field extraction review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use field extraction review prompt as a checklist item before the agent produces a final answer.
- Use field extraction review prompt during review to separate supported observations from unresolved risks.
- Use field extraction review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating field extraction review prompt as permission to invent missing details.
