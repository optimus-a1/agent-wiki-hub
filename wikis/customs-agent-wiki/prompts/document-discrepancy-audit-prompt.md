---
title: "Document Discrepancy Audit Prompt"
...
# Document Discrepancy Audit Prompt

## Purpose
This page gives agents a stable prompt pattern for document discrepancy audit prompt in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Document Discrepancy Audit Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use document discrepancy audit prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use document discrepancy audit prompt as a checklist item before the agent produces a final answer.
- Use document discrepancy audit prompt during review to separate supported observations from unresolved risks.
- Use document discrepancy audit prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating document discrepancy audit prompt as permission to invent missing details.
