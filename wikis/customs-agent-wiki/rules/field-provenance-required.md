---
title: "Field Provenance Required"
...
## Purpose
This page gives agents a stable rule pattern for field provenance required in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Field Provenance Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use field provenance required to convert a vague request into explicit fields, checks, and boundaries.
- Use field provenance required as a checklist item before the agent produces a final answer.
- Use field provenance required during review to separate supported observations from unresolved risks.
- Use field provenance required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating field provenance required as permission to invent missing details.
