---
title: "Weight And Package Reconciliation"
...
# Weight And Package Reconciliation

## Purpose
This page gives agents a stable rule pattern for weight and package reconciliation in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Weight And Package Reconciliation should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use weight and package reconciliation to convert a vague request into explicit fields, checks, and boundaries.
- Use weight and package reconciliation as a checklist item before the agent produces a final answer.
- Use weight and package reconciliation during review to separate supported observations from unresolved risks.
- Use weight and package reconciliation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating weight and package reconciliation as permission to invent missing details.
