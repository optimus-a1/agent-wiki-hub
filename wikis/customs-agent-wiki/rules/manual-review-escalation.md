...
## Purpose
This page gives agents a stable rule pattern for manual review escalation in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Manual Review Escalation should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use manual review escalation to convert a vague request into explicit fields, checks, and boundaries.
- Use manual review escalation as a checklist item before the agent produces a final answer.
- Use manual review escalation during review to separate supported observations from unresolved risks.
- Use manual review escalation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating manual review escalation as permission to invent missing details.
