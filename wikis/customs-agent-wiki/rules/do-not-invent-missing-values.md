---
title: "Do Not Invent Missing Values"
...
## Purpose
This page gives agents a stable rule pattern for do not invent missing values in `customs-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Do Not Invent Missing Values should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use do not invent missing values to convert a vague request into explicit fields, checks, and boundaries.
- Use do not invent missing values as a checklist item before the agent produces a final answer.
- Use do not invent missing values during review to separate supported observations from unresolved risks.
- Use do not invent missing values to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating do not invent missing values as permission to invent missing details.
