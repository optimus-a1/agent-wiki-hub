...
## Purpose
This page gives agents a stable rule pattern for jurisdiction required in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Jurisdiction Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use jurisdiction required to convert a vague request into explicit fields, checks, and boundaries.
- Use jurisdiction required as a checklist item before the agent produces a final answer.
- Use jurisdiction required during review to separate supported observations from unresolved risks.
- Use jurisdiction required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating jurisdiction required as permission to invent missing details.
