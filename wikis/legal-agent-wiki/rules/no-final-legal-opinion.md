...
## Purpose
This page gives agents a stable rule pattern for no final legal opinion in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Final Legal Opinion should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no final legal opinion to convert a vague request into explicit fields, checks, and boundaries.
- Use no final legal opinion as a checklist item before the agent produces a final answer.
- Use no final legal opinion during review to separate supported observations from unresolved risks.
- Use no final legal opinion to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no final legal opinion as permission to invent missing details.
