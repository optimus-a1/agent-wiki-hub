...
## Purpose
This page gives agents a stable rule pattern for lawyer review required in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Lawyer Review Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use lawyer review required to convert a vague request into explicit fields, checks, and boundaries.
- Use lawyer review required as a checklist item before the agent produces a final answer.
- Use lawyer review required during review to separate supported observations from unresolved risks.
- Use lawyer review required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating lawyer review required as permission to invent missing details.
