...
## Purpose
This page gives agents a stable rule pattern for uncertainty disclosure in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Uncertainty Disclosure should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use uncertainty disclosure to convert a vague request into explicit fields, checks, and boundaries.
- Use uncertainty disclosure as a checklist item before the agent produces a final answer.
- Use uncertainty disclosure during review to separate supported observations from unresolved risks.
- Use uncertainty disclosure to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating uncertainty disclosure as permission to invent missing details.
