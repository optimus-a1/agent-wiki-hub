...
## Purpose
This page gives agents a stable rule pattern for current law source gate in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Current Law Source Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use current law source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use current law source gate as a checklist item before the agent produces a final answer.
- Use current law source gate during review to separate supported observations from unresolved risks.
- Use current law source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating current law source gate as permission to invent missing details.
