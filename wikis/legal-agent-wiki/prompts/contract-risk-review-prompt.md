---
title: "Contract Risk Review Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for contract risk review prompt in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Contract Risk Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use contract risk review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use contract risk review prompt as a checklist item before the agent produces a final answer.
- Use contract risk review prompt during review to separate supported observations from unresolved risks.
- Use contract risk review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating contract risk review prompt as permission to invent missing details.
