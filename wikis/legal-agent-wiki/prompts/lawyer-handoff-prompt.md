...
# Lawyer Handoff Prompt

## Purpose
This page gives agents a stable prompt pattern for lawyer handoff prompt in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Lawyer Handoff Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use lawyer handoff prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use lawyer handoff prompt as a checklist item before the agent produces a final answer.
- Use lawyer handoff prompt during review to separate supported observations from unresolved risks.
- Use lawyer handoff prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating lawyer handoff prompt as permission to invent missing details.
