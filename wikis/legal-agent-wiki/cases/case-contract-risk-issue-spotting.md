---
title: "Case Contract Risk Issue Spotting"
...
# Case Contract Risk Issue Spotting

## Purpose
This page gives agents a stable case pattern for case contract risk issue spotting in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Contract Risk Issue Spotting should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case contract risk issue spotting to convert a vague request into explicit fields, checks, and boundaries.
- Use case contract risk issue spotting as a checklist item before the agent produces a final answer.
- Use case contract risk issue spotting during review to separate supported observations from unresolved risks.
- Use case contract risk issue spotting to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case contract risk issue spotting as permission to invent missing details.
