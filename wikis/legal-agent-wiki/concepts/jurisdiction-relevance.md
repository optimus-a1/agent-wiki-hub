...
## Purpose
This page gives agents a stable concept pattern for jurisdiction relevance in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Jurisdiction Relevance should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use jurisdiction relevance to convert a vague request into explicit fields, checks, and boundaries.
- Use jurisdiction relevance as a checklist item before the agent produces a final answer.
- Use jurisdiction relevance during review to separate supported observations from unresolved risks.
- Use jurisdiction relevance to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating jurisdiction relevance as permission to invent missing details.
