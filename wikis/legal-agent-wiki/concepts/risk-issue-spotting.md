...
## Purpose
This page gives agents a stable concept pattern for risk issue spotting in `legal-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Risk Issue Spotting should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use risk issue spotting to convert a vague request into explicit fields, checks, and boundaries.
- Use risk issue spotting as a checklist item before the agent produces a final answer.
- Use risk issue spotting during review to separate supported observations from unresolved risks.
- Use risk issue spotting to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating risk issue spotting as permission to invent missing details.
