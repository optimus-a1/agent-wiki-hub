---
title: "Dependency Change Boundary"
...
## Purpose
This page gives agents a stable rule pattern for dependency change boundary in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dependency Change Boundary should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use dependency change boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use dependency change boundary as a checklist item before the agent produces a final answer.
- Use dependency change boundary during review to separate supported observations from unresolved risks.
- Use dependency change boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dependency change boundary as permission to invent missing details.
