...
## Purpose
This page gives agents a stable rule pattern for scope before editing in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Scope Before Editing should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use scope before editing to convert a vague request into explicit fields, checks, and boundaries.
- Use scope before editing as a checklist item before the agent produces a final answer.
- Use scope before editing during review to separate supported observations from unresolved risks.
- Use scope before editing to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating scope before editing as permission to invent missing details.
