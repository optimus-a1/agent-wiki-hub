...
## Purpose
This page gives agents a stable rule pattern for no secret in repo in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Secret In Repo should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no secret in repo to convert a vague request into explicit fields, checks, and boundaries.
- Use no secret in repo as a checklist item before the agent produces a final answer.
- Use no secret in repo during review to separate supported observations from unresolved risks.
- Use no secret in repo to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no secret in repo as permission to invent missing details.
