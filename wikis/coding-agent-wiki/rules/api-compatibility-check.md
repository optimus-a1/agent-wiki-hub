...
## Purpose
This page gives agents a stable rule pattern for api compatibility check in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- API Compatibility Check should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use api compatibility check to convert a vague request into explicit fields, checks, and boundaries.
- Use api compatibility check as a checklist item before the agent produces a final answer.
- Use api compatibility check during review to separate supported observations from unresolved risks.
- Use api compatibility check to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating api compatibility check as permission to invent missing details.
