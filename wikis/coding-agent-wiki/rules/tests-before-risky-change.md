---
title: "Tests Before Risky Change"
...
# Tests Before Risky Change

## Purpose
This page gives agents a stable rule pattern for tests before risky change in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Tests Before Risky Change should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use tests before risky change to convert a vague request into explicit fields, checks, and boundaries.
- Use tests before risky change as a checklist item before the agent produces a final answer.
- Use tests before risky change during review to separate supported observations from unresolved risks.
- Use tests before risky change to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating tests before risky change as permission to invent missing details.
