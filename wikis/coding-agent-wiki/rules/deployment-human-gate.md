...
## Purpose
This page gives agents a stable rule pattern for deployment human gate in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Deployment Human Gate should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use deployment human gate to convert a vague request into explicit fields, checks, and boundaries.
- Use deployment human gate as a checklist item before the agent produces a final answer.
- Use deployment human gate during review to separate supported observations from unresolved risks.
- Use deployment human gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating deployment human gate as permission to invent missing details.
