...
## Purpose
This page gives agents a stable rule pattern for preserve user changes in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Preserve User Changes should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use preserve user changes to convert a vague request into explicit fields, checks, and boundaries.
- Use preserve user changes as a checklist item before the agent produces a final answer.
- Use preserve user changes during review to separate supported observations from unresolved risks.
- Use preserve user changes to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating preserve user changes as permission to invent missing details.
