...
## Purpose
This page gives agents a stable rule pattern for error visibility rule in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Error Visibility Rule should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use error visibility rule to convert a vague request into explicit fields, checks, and boundaries.
- Use error visibility rule as a checklist item before the agent produces a final answer.
- Use error visibility rule during review to separate supported observations from unresolved risks.
- Use error visibility rule to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating error visibility rule as permission to invent missing details.
