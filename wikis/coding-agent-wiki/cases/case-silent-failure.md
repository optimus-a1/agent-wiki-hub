...
## Purpose
This page gives agents a stable case pattern for case silent failure in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Silent Failure should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case silent failure to convert a vague request into explicit fields, checks, and boundaries.
- Use case silent failure as a checklist item before the agent produces a final answer.
- Use case silent failure during review to separate supported observations from unresolved risks.
- Use case silent failure to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case silent failure as permission to invent missing details.
