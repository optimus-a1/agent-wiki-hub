...
# Case Secret In Config

## Purpose
This page gives agents a stable case pattern for case secret in config in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Secret In Config should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case secret in config to convert a vague request into explicit fields, checks, and boundaries.
- Use case secret in config as a checklist item before the agent produces a final answer.
- Use case secret in config during review to separate supported observations from unresolved risks.
- Use case secret in config to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case secret in config as permission to invent missing details.
