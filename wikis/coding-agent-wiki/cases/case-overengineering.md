...
## Purpose
This page gives agents a stable case pattern for case overengineering in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Overengineering should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case overengineering to convert a vague request into explicit fields, checks, and boundaries.
- Use case overengineering as a checklist item before the agent produces a final answer.
- Use case overengineering during review to separate supported observations from unresolved risks.
- Use case overengineering to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case overengineering as permission to invent missing details.
