---
title: "Case Missing Regression Test"
...
# Case Missing Regression Test

## Purpose
This page gives agents a stable case pattern for case missing regression test in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Missing Regression Test should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case missing regression test to convert a vague request into explicit fields, checks, and boundaries.
- Use case missing regression test as a checklist item before the agent produces a final answer.
- Use case missing regression test during review to separate supported observations from unresolved risks.
- Use case missing regression test to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case missing regression test as permission to invent missing details.
