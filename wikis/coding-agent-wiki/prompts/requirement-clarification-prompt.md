---
title: "Requirement Clarification Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for requirement clarification prompt in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Requirement Clarification Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use requirement clarification prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use requirement clarification prompt as a checklist item before the agent produces a final answer.
- Use requirement clarification prompt during review to separate supported observations from unresolved risks.
- Use requirement clarification prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating requirement clarification prompt as permission to invent missing details.
