---
title: "Deployment Readiness Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for deployment readiness prompt in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Deployment Readiness Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use deployment readiness prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use deployment readiness prompt as a checklist item before the agent produces a final answer.
- Use deployment readiness prompt during review to separate supported observations from unresolved risks.
- Use deployment readiness prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating deployment readiness prompt as permission to invent missing details.
