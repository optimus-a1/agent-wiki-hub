---
title: "Minimal Implementation Prompt"
...
## Purpose
This page gives agents a stable prompt pattern for minimal implementation prompt in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Minimal Implementation Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use minimal implementation prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use minimal implementation prompt as a checklist item before the agent produces a final answer.
- Use minimal implementation prompt during review to separate supported observations from unresolved risks.
- Use minimal implementation prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating minimal implementation prompt as permission to invent missing details.
