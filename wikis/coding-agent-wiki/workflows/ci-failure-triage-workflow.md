...
## Purpose
This page gives agents a stable workflow pattern for ci failure triage workflow in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- CI Failure Triage Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use ci failure triage workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use ci failure triage workflow as a checklist item before the agent produces a final answer.
- Use ci failure triage workflow during review to separate supported observations from unresolved risks.
- Use ci failure triage workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating ci failure triage workflow as permission to invent missing details.
