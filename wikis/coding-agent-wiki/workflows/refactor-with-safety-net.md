...
## Purpose
This page gives agents a stable workflow pattern for refactor with safety net in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Refactor With Safety Net should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use refactor with safety net to convert a vague request into explicit fields, checks, and boundaries.
- Use refactor with safety net as a checklist item before the agent produces a final answer.
- Use refactor with safety net during review to separate supported observations from unresolved risks.
- Use refactor with safety net to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating refactor with safety net as permission to invent missing details.
