---
title: "Deployment Preflight Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for deployment preflight workflow in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Deployment Preflight Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use deployment preflight workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use deployment preflight workflow as a checklist item before the agent produces a final answer.
- Use deployment preflight workflow during review to separate supported observations from unresolved risks.
- Use deployment preflight workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating deployment preflight workflow as permission to invent missing details.
