---
title: "Test First Implementation Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for test first implementation workflow in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Test First Implementation Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use test first implementation workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use test first implementation workflow as a checklist item before the agent produces a final answer.
- Use test first implementation workflow during review to separate supported observations from unresolved risks.
- Use test first implementation workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating test first implementation workflow as permission to invent missing details.
