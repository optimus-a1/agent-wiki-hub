---
title: "Requirement To Task Workflow"
...
## Purpose
This page gives agents a stable workflow pattern for requirement to task workflow in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Requirement To Task Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use requirement to task workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use requirement to task workflow as a checklist item before the agent produces a final answer.
- Use requirement to task workflow during review to separate supported observations from unresolved risks.
- Use requirement to task workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating requirement to task workflow as permission to invent missing details.
