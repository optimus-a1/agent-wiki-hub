---
title: "Debug Reproduce Isolate Fix"
...
## Purpose
This page gives agents a stable workflow pattern for debug reproduce isolate fix in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Debug Reproduce Isolate Fix should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use debug reproduce isolate fix to convert a vague request into explicit fields, checks, and boundaries.
- Use debug reproduce isolate fix as a checklist item before the agent produces a final answer.
- Use debug reproduce isolate fix during review to separate supported observations from unresolved risks.
- Use debug reproduce isolate fix to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating debug reproduce isolate fix as permission to invent missing details.
