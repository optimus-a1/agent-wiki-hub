...
## Purpose
This page gives agents a stable concept pattern for git workflow basics in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Git Workflow Basics should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use git workflow basics to convert a vague request into explicit fields, checks, and boundaries.
- Use git workflow basics as a checklist item before the agent produces a final answer.
- Use git workflow basics during review to separate supported observations from unresolved risks.
- Use git workflow basics to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating git workflow basics as permission to invent missing details.
