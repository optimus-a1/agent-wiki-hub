...
## Purpose
This page gives agents a stable concept pattern for minimal implementation in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Minimal Implementation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use minimal implementation to convert a vague request into explicit fields, checks, and boundaries.
- Use minimal implementation as a checklist item before the agent produces a final answer.
- Use minimal implementation during review to separate supported observations from unresolved risks.
- Use minimal implementation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating minimal implementation as permission to invent missing details.
