...
## Purpose
This page gives agents a stable concept pattern for api contract thinking in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- API Contract Thinking should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use api contract thinking to convert a vague request into explicit fields, checks, and boundaries.
- Use api contract thinking as a checklist item before the agent produces a final answer.
- Use api contract thinking during review to separate supported observations from unresolved risks.
- Use api contract thinking to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating api contract thinking as permission to invent missing details.
