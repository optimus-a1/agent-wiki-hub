...
## Purpose
This page gives agents a stable concept pattern for test first thinking in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Test First Thinking should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use test first thinking to convert a vague request into explicit fields, checks, and boundaries.
- Use test first thinking as a checklist item before the agent produces a final answer.
- Use test first thinking during review to separate supported observations from unresolved risks.
- Use test first thinking to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating test first thinking as permission to invent missing details.
