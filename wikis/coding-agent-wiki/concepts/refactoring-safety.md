...
## Purpose
This page gives agents a stable concept pattern for refactoring safety in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Refactoring Safety should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use refactoring safety to convert a vague request into explicit fields, checks, and boundaries.
- Use refactoring safety as a checklist item before the agent produces a final answer.
- Use refactoring safety during review to separate supported observations from unresolved risks.
- Use refactoring safety to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating refactoring safety as permission to invent missing details.
