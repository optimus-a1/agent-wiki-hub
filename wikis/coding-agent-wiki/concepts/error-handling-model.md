...
## Purpose
This page gives agents a stable concept pattern for error handling model in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Error Handling Model should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use error handling model to convert a vague request into explicit fields, checks, and boundaries.
- Use error handling model as a checklist item before the agent produces a final answer.
- Use error handling model during review to separate supported observations from unresolved risks.
- Use error handling model to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating error handling model as permission to invent missing details.
