...
## Purpose
This page gives agents a stable concept pattern for dependency boundary in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Dependency Boundary should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use dependency boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use dependency boundary as a checklist item before the agent produces a final answer.
- Use dependency boundary during review to separate supported observations from unresolved risks.
- Use dependency boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating dependency boundary as permission to invent missing details.
