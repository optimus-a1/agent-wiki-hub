...
## Purpose
This page gives agents a stable concept pattern for requirement clarification in `coding-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Requirement Clarification should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use requirement clarification to convert a vague request into explicit fields, checks, and boundaries.
- Use requirement clarification as a checklist item before the agent produces a final answer.
- Use requirement clarification during review to separate supported observations from unresolved risks.
- Use requirement clarification to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating requirement clarification as permission to invent missing details.
