...
## Purpose
This page gives agents a stable rule pattern for paper trading default in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Paper Trading Default should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use paper trading default to convert a vague request into explicit fields, checks, and boundaries.
- Use paper trading default as a checklist item before the agent produces a final answer.
- Use paper trading default during review to separate supported observations from unresolved risks.
- Use paper trading default to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating paper trading default as permission to invent missing details.
