...
# Out Of Sample Required

## Purpose
This page gives agents a stable rule pattern for out of sample required in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Out Of Sample Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use out of sample required to convert a vague request into explicit fields, checks, and boundaries.
- Use out of sample required as a checklist item before the agent produces a final answer.
- Use out of sample required during review to separate supported observations from unresolved risks.
- Use out of sample required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating out of sample required as permission to invent missing details.
