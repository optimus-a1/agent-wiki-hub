...
# Backtest Costs Required

## Purpose
This page gives agents a stable rule pattern for backtest costs required in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Backtest Costs Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use backtest costs required to convert a vague request into explicit fields, checks, and boundaries.
- Use backtest costs required as a checklist item before the agent produces a final answer.
- Use backtest costs required during review to separate supported observations from unresolved risks.
- Use backtest costs required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating backtest costs required as permission to invent missing details.
