---
title: "Trading System Permission Control"
...
# Trading System Permission Control

## Purpose
This page gives agents a stable rule pattern for trading system permission control in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Trading System Permission Control should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use trading system permission control to convert a vague request into explicit fields, checks, and boundaries.
- Use trading system permission control as a checklist item before the agent produces a final answer.
- Use trading system permission control during review to separate supported observations from unresolved risks.
- Use trading system permission control to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating trading system permission control as permission to invent missing details.
