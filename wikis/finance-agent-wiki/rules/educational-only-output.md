...
## Purpose
This page gives agents a stable rule pattern for educational only output in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Educational Only Output should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use educational only output to convert a vague request into explicit fields, checks, and boundaries.
- Use educational only output as a checklist item before the agent produces a final answer.
- Use educational only output during review to separate supported observations from unresolved risks.
- Use educational only output to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating educational only output as permission to invent missing details.
