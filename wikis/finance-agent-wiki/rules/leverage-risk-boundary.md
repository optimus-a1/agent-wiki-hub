...
# Leverage Risk Boundary

## Purpose
This page gives agents a stable rule pattern for leverage risk boundary in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Leverage Risk Boundary should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use leverage risk boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use leverage risk boundary as a checklist item before the agent produces a final answer.
- Use leverage risk boundary during review to separate supported observations from unresolved risks.
- Use leverage risk boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating leverage risk boundary as permission to invent missing details.
