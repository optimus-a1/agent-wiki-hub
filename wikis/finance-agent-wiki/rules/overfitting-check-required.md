...
# Overfitting Check Required

## Purpose
This page gives agents a stable rule pattern for overfitting check required in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Overfitting Check Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use overfitting check required to convert a vague request into explicit fields, checks, and boundaries.
- Use overfitting check required as a checklist item before the agent produces a final answer.
- Use overfitting check required during review to separate supported observations from unresolved risks.
- Use overfitting check required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating overfitting check required as permission to invent missing details.
