...
## Purpose
This page gives agents a stable concept pattern for position sizing model in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Position Sizing Model should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use position sizing model to convert a vague request into explicit fields, checks, and boundaries.
- Use position sizing model as a checklist item before the agent produces a final answer.
- Use position sizing model during review to separate supported observations from unresolved risks.
- Use position sizing model to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating position sizing model as permission to invent missing details.
