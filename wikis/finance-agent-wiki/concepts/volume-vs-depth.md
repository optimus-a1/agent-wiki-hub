...
## Purpose
This page gives agents a stable concept pattern for volume vs depth in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Volume Vs Depth should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use volume vs depth to convert a vague request into explicit fields, checks, and boundaries.
- Use volume vs depth as a checklist item before the agent produces a final answer.
- Use volume vs depth during review to separate supported observations from unresolved risks.
- Use volume vs depth to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating volume vs depth as permission to invent missing details.
