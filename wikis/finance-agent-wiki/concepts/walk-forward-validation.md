...
## Purpose
This page gives agents a stable concept pattern for walk forward validation in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Walk Forward Validation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use walk forward validation to convert a vague request into explicit fields, checks, and boundaries.
- Use walk forward validation as a checklist item before the agent produces a final answer.
- Use walk forward validation during review to separate supported observations from unresolved risks.
- Use walk forward validation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating walk forward validation as permission to invent missing details.
