...
## Purpose
This page gives agents a stable concept pattern for look ahead bias in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Look Ahead Bias should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use look ahead bias to convert a vague request into explicit fields, checks, and boundaries.
- Use look ahead bias as a checklist item before the agent produces a final answer.
- Use look ahead bias during review to separate supported observations from unresolved risks.
- Use look ahead bias to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating look ahead bias as permission to invent missing details.
