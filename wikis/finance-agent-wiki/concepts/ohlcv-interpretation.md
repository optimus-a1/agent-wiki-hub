...
## Purpose
This page gives agents a stable concept pattern for ohlcv interpretation in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Ohlcv Interpretation should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use ohlcv interpretation to convert a vague request into explicit fields, checks, and boundaries.
- Use ohlcv interpretation as a checklist item before the agent produces a final answer.
- Use ohlcv interpretation during review to separate supported observations from unresolved risks.
- Use ohlcv interpretation to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating ohlcv interpretation as permission to invent missing details.
