...
## Purpose
This page gives agents a stable concept pattern for order book basics in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Order Book Basics should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use order book basics to convert a vague request into explicit fields, checks, and boundaries.
- Use order book basics as a checklist item before the agent produces a final answer.
- Use order book basics during review to separate supported observations from unresolved risks.
- Use order book basics to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating order book basics as permission to invent missing details.
