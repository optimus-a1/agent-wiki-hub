---
title: "Backtest Design Workflow"
...
# Backtest Design Workflow

## Purpose
This page gives agents a stable workflow pattern for backtest design workflow in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Backtest Design Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use backtest design workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use backtest design workflow as a checklist item before the agent produces a final answer.
- Use backtest design workflow during review to separate supported observations from unresolved risks.
- Use backtest design workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating backtest design workflow as permission to invent missing details.
