...
# Backtest Audit Prompt

## Purpose
This page gives agents a stable prompt pattern for backtest audit prompt in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Backtest Audit Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use backtest audit prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use backtest audit prompt as a checklist item before the agent produces a final answer.
- Use backtest audit prompt during review to separate supported observations from unresolved risks.
- Use backtest audit prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating backtest audit prompt as permission to invent missing details.
