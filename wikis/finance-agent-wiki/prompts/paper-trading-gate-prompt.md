---
title: "Paper Trading Gate Prompt"
...
# Paper Trading Gate Prompt

## Purpose
This page gives agents a stable prompt pattern for paper trading gate prompt in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Paper Trading Gate Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use paper trading gate prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use paper trading gate prompt as a checklist item before the agent produces a final answer.
- Use paper trading gate prompt during review to separate supported observations from unresolved risks.
- Use paper trading gate prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating paper trading gate prompt as permission to invent missing details.
