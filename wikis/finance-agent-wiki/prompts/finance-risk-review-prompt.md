---
title: "Finance Risk Review Prompt"
...
# Finance Risk Review Prompt

## Purpose
This page gives agents a stable prompt pattern for finance risk review prompt in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Finance Risk Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use finance risk review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use finance risk review prompt as a checklist item before the agent produces a final answer.
- Use finance risk review prompt during review to separate supported observations from unresolved risks.
- Use finance risk review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating finance risk review prompt as permission to invent missing details.
