---
title: "Case Backtest Overfitting"
...
# Case Backtest Overfitting

## Purpose
This page gives agents a stable case pattern for case backtest overfitting in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Backtest Overfitting should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case backtest overfitting to convert a vague request into explicit fields, checks, and boundaries.
- Use case backtest overfitting as a checklist item before the agent produces a final answer.
- Use case backtest overfitting during review to separate supported observations from unresolved risks.
- Use case backtest overfitting to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case backtest overfitting as permission to invent missing details.
