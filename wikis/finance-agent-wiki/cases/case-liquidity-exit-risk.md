---
title: "Case Liquidity Exit Risk"
...
# Case Liquidity Exit Risk

## Purpose
This page gives agents a stable case pattern for case liquidity exit risk in `finance-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Liquidity Exit Risk should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case liquidity exit risk to convert a vague request into explicit fields, checks, and boundaries.
- Use case liquidity exit risk as a checklist item before the agent produces a final answer.
- Use case liquidity exit risk during review to separate supported observations from unresolved risks.
- Use case liquidity exit risk to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case liquidity exit risk as permission to invent missing details.
