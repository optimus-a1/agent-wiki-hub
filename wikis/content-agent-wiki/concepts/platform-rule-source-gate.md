---
title: "Platform Rule Source Gate"
...
# Platform Rule Source Gate

## Purpose
This page gives agents a stable concept pattern for platform rule source gate in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Platform Rule Source Gate should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use platform rule source gate to convert a vague request into explicit fields, checks, and boundaries.
- Use platform rule source gate as a checklist item before the agent produces a final answer.
- Use platform rule source gate during review to separate supported observations from unresolved risks.
- Use platform rule source gate to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating platform rule source gate as permission to invent missing details.
