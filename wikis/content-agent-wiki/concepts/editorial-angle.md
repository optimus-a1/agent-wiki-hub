...
## Purpose
This page gives agents a stable concept pattern for editorial angle in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Editorial Angle should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use editorial angle to convert a vague request into explicit fields, checks, and boundaries.
- Use editorial angle as a checklist item before the agent produces a final answer.
- Use editorial angle during review to separate supported observations from unresolved risks.
- Use editorial angle to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating editorial angle as permission to invent missing details.
