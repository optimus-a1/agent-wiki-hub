...
## Purpose
This page gives agents a stable concept pattern for audience and format fit in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Audience And Format Fit should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use audience and format fit to convert a vague request into explicit fields, checks, and boundaries.
- Use audience and format fit as a checklist item before the agent produces a final answer.
- Use audience and format fit during review to separate supported observations from unresolved risks.
- Use audience and format fit to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating audience and format fit as permission to invent missing details.
