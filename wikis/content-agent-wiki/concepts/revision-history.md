...
## Purpose
This page gives agents a stable concept pattern for revision history in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Revision History should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use revision history to convert a vague request into explicit fields, checks, and boundaries.
- Use revision history as a checklist item before the agent produces a final answer.
- Use revision history during review to separate supported observations from unresolved risks.
- Use revision history to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating revision history as permission to invent missing details.
