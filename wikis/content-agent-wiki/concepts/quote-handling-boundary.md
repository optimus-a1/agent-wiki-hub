...
# Quote Handling Boundary

## Purpose
This page gives agents a stable concept pattern for quote handling boundary in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Quote Handling Boundary should be handled as concept knowledge that supports repeatable agent behavior.
...
## Examples
- Use quote handling boundary to convert a vague request into explicit fields, checks, and boundaries.
- Use quote handling boundary as a checklist item before the agent produces a final answer.
- Use quote handling boundary during review to separate supported observations from unresolved risks.
- Use quote handling boundary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating quote handling boundary as permission to invent missing details.
