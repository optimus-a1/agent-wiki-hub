...
# Case Overlong Summary

## Purpose
This page gives agents a stable case pattern for case overlong summary in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Overlong Summary should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case overlong summary to convert a vague request into explicit fields, checks, and boundaries.
- Use case overlong summary as a checklist item before the agent produces a final answer.
- Use case overlong summary during review to separate supported observations from unresolved risks.
- Use case overlong summary to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case overlong summary as permission to invent missing details.
