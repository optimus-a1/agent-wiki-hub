...
## Purpose
This page gives agents a stable case pattern for case invented quote in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Case Invented Quote should be handled as case knowledge that supports repeatable agent behavior.
...
## Examples
- Use case invented quote to convert a vague request into explicit fields, checks, and boundaries.
- Use case invented quote as a checklist item before the agent produces a final answer.
- Use case invented quote during review to separate supported observations from unresolved risks.
- Use case invented quote to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating case invented quote as permission to invent missing details.
