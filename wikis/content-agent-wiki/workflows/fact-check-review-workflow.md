---
title: "Fact Check Review Workflow"
...
# Fact Check Review Workflow

## Purpose
This page gives agents a stable workflow pattern for fact check review workflow in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Fact Check Review Workflow should be handled as workflow knowledge that supports repeatable agent behavior.
...
## Examples
- Use fact check review workflow to convert a vague request into explicit fields, checks, and boundaries.
- Use fact check review workflow as a checklist item before the agent produces a final answer.
- Use fact check review workflow during review to separate supported observations from unresolved risks.
- Use fact check review workflow to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating fact check review workflow as permission to invent missing details.
