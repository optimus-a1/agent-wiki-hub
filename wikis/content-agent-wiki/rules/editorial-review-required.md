...
## Purpose
This page gives agents a stable rule pattern for editorial review required in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Editorial Review Required should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use editorial review required to convert a vague request into explicit fields, checks, and boundaries.
- Use editorial review required as a checklist item before the agent produces a final answer.
- Use editorial review required during review to separate supported observations from unresolved risks.
- Use editorial review required to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating editorial review required as permission to invent missing details.
