---
title: "Source Required For Factual Claims"
...
## Purpose
This page gives agents a stable rule pattern for source required for factual claims in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Source Required For Factual Claims should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use source required for factual claims to convert a vague request into explicit fields, checks, and boundaries.
- Use source required for factual claims as a checklist item before the agent produces a final answer.
- Use source required for factual claims during review to separate supported observations from unresolved risks.
- Use source required for factual claims to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating source required for factual claims as permission to invent missing details.
