...
## Purpose
This page gives agents a stable rule pattern for no invented quotes in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- No Invented Quotes should be handled as rule knowledge that supports repeatable agent behavior.
...
## Examples
- Use no invented quotes to convert a vague request into explicit fields, checks, and boundaries.
- Use no invented quotes as a checklist item before the agent produces a final answer.
- Use no invented quotes during review to separate supported observations from unresolved risks.
- Use no invented quotes to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating no invented quotes as permission to invent missing details.
