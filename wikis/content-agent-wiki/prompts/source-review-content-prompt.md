---
title: "Source Review Content Prompt"
...
# Source Review Content Prompt

## Purpose
This page gives agents a stable prompt pattern for source review content prompt in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Source Review Content Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use source review content prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use source review content prompt as a checklist item before the agent produces a final answer.
- Use source review content prompt during review to separate supported observations from unresolved risks.
- Use source review content prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating source review content prompt as permission to invent missing details.
