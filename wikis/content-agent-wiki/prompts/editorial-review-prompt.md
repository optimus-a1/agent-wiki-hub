...
## Purpose
This page gives agents a stable prompt pattern for editorial review prompt in `content-agent-wiki`. It is model-synthesized stable knowledge, not an authoritative source and not a current-fact record.

## Stable Knowledge
- Editorial Review Prompt should be handled as prompt knowledge that supports repeatable agent behavior.
...
## Examples
- Use editorial review prompt to convert a vague request into explicit fields, checks, and boundaries.
- Use editorial review prompt as a checklist item before the agent produces a final answer.
- Use editorial review prompt during review to separate supported observations from unresolved risks.
- Use editorial review prompt to decide whether a human gate or source gate is required.

## Anti-Patterns
- Treating editorial review prompt as permission to invent missing details.
