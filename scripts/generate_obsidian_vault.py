...
def write_dashboards(records: list[dict]) -> list[str]:
    ...
    density = json.loads(read_text(REGISTRY / "knowledge-density-report.json") or "{}")
    leakage = json.loads(read_text(REGISTRY / "current-fact-leakage-audit.json") or "{}")
    boundaries = json.loads(read_text(REGISTRY / "high-risk-boundary-audit.json") or "{}")
    ...
    for record in density.get("records", []):
        c = record.get("counts", {})
        density_table.append(
            f"| {record.get('wiki')} | {record.get('knowledge_density_group')} | {c.get('concepts', 0)} | "
            f"{c.get('rules', 0)} | {c.get('workflows', 0)} | {c.get('cases', 0)} | "
            f"{c.get('prompts', 0)} | {record.get('eval_tests', 0)} |"
        )
    pages = {
        "Wiki Status.md": [
            "# Wiki Status",
        ...
        "Knowledge Density.md": [
            "# Knowledge Density",
            "",
            *(density_table if len(density_table) > 2 else ["Knowledge density report has not been generated yet."]),
            "",
            "## Optional Dataview",
            "```dataview",
            "TABLE wiki, risk, source_status FROM \"02_Knowledge/MOCs\"",
        ...
        "High Risk Boundaries.md": [
            ...
            f"- v2.1 pages checked: {len(boundaries.get('checks', []))}",
            f"- Legacy warnings: {len(boundaries.get('legacy_warnings', []))}",
            "",
            "| Wiki | Human Gate | Source Gate |",
            "| --- | --- | --- |",
            *[
                f"| {check.get('wiki')} | {check.get('has_human_gate')} | {check.get('has_source_gate')} |"
                for check in boundaries.get("wiki_checks", [])
            ],
        ],
    }
    written = []
...
def write_docs(records: list[dict], mocs: list[str], dashboards: list[str]) -> None:
    ...
    (DOCS / "OBSIDIAN_USAGE.md").write_text(
        ...
        "v2.1 adds `Knowledge Density.md`, `Current Fact Gates.md`, `Human Review Gates.md`, "
        "`High Risk Boundaries.md`, and `Knowledge Density Map.canvas`. All pages are readable "
        "as plain Markdown; Dataview and Bases are optional.\n",
        encoding="utf-8",
    )
